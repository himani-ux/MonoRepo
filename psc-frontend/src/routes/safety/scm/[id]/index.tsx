...
export default function SafetyScmDetailRoute() {
  ...
  const [officeReviewed, setOfficeReviewed] = useState(true);
  ...
  useEffect(() => {
    ...
    setOfficeReviewed(existingComment.trim() ? Boolean(meeting.is_reviewed ?? meeting.office_comment_at) : true);
    ...
    <section className="space-y-6">
      ...
      <header className="rounded-3xl border border-slate-200 bg-[linear-gradient(135deg,#f8fafc_0%,#ffffff_55%,#fef9c3_100%)] p-6 shadow-sm">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
          Meeting record for the locked 10-section SCM structure with attendance,
          ...
          <div className="flex flex-wrap gap-3">
            {canOpenSignoff ? (
              <Link
                ...
                to={`/safety/scm/${meeting.public_id ?? meeting.id}/signoff`}
              >
                Open Master Sign-Off
              </Link>
            ) : null}
            {pdfIsSigned ? (
              <button
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-3 text-xs text-slate-500">
            Reviewed by {meeting.office_comment_by ?? "Not recorded"} · {formatDateTime(meeting.office_comment_at)}
          </p>
        </div>
        ...
        {officeReviewEditable ? (
          <div className="mt-5 space-y-4">
            ...
            <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
              <input
                checked={officeReviewed}
                className="mt-1 h-4 w-4"
                onChange={(event) => {
                  setOfficeReviewed(event.target.checked);
                  setOfficeReviewSaved(false);
                }}
                type="checkbox"
              />
              <span>
                Mark IsReviewed = true for legacy Section 10 office review.
              </span>
            </label>
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                disabled={officeReviewPending || !officeCommentDraft.trim() || !officeReviewed}
                onClick={() => void handleOfficeReview()}
                type="button"
              >
                {officeReviewPending ? "Saving office review..." : "Save Section 10 Office Review"}
              </button>
            </div>
          </div>
        ) : (
          <p className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
            {officeReviewAvailable
              ...
              : "Section 10 becomes editable after Master sign-off."}
          </p>
        )}
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Agenda routes</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              The dedicated review routes read the same meeting state, agenda decisions, and summary counts.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              ...
              to={`/safety/scm/${meeting.public_id ?? meeting.id}/agenda`}
            >
              Open agenda route
            </Link>
            <Link
              className="inline-flex min-h-[44px] items-center rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700"
              to={`/safety/scm/${meeting.public_id ?? meeting.id}/closed-since-last`}
            >
              Open closed-since-last route
            </Link>
            {canOpenSignoff ? (
              <Link
                className="inline-flex min-h-[44px] items-center rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700"
