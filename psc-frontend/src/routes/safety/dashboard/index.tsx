...
function DashboardPanelLoadingState({ className = "h-36" }: { className?: string }) {
  return (
    <section aria-label="Loading dashboard panel" className="space-y-3" role="status">
      <div className={`${className} animate-pulse rounded-lg border border-sky-100 bg-sky-50`} />
    </section>
  );
}
...
export default function SafetyDashboardRoute() {
  ...
  return (
    <section className="space-y-6">
      <header className="rounded-lg border border-sky-100 bg-sky-50/70 p-5 shadow-sm">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              What needs attention and what is overdue.
            </p>
            <p className="mt-3 text-sm font-medium text-slate-700">Scope: {scopeSummary}</p>
          </div>
            ...
            {hasVesselSelector ? (
              <label className="flex flex-col gap-2 text-sm font-medium text-slate-700">
                ...
                <select
                  ...
                  className="min-w-[18rem] rounded-md border border-sky-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm outline-none transition focus:border-teal-500"
                  ...
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                      isActive
                        ...
                        : "border border-sky-200 bg-white text-slate-700 hover:border-teal-300"
                    }`}
        ...
        <SafetyCompositeScoreCard
          ...
          metrics={{
            ...
            totalCorrectiveActions:
              compositeQuery.data.metrics.total_corrective_actions ??
              compositeQuery.data.metrics.overdue_corrective_actions,
            totalFindings:
              compositeQuery.data.metrics.total_findings ??
              compositeQuery.data.metrics.open_findings,
            totalIncidents:
              compositeQuery.data.metrics.total_incidents ??
              compositeQuery.data.metrics.open_incidents,
            totalNearMisses:
              compositeQuery.data.metrics.total_near_misses ??
              compositeQuery.data.metrics.open_near_misses,
          }}
      ...
      <section className="rounded-lg border border-sky-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              ...
              {compositeQuery.data
                ...
                : "Loading"}
              {repeatRootQuery.data ? ` | Repeat level: ${repeatRootQuery.data.minimum_repeat_count}+` : ""}
              {caAgingQuery.data ? ` | Oldest open action: ${caAgingQuery.data.oldest_age_days} day(s)` : ""}
            </p>
          </div>
          <button
            aria-expanded={showAdvancedCards}
            className="rounded-md border border-sky-200 bg-sky-50 px-5 py-3 text-sm font-semibold text-teal-800 transition hover:border-teal-300 hover:bg-teal-50"
            onClick={() => setShowAdvancedCards((value) => !value)}
            type="button"
          >
            {showAdvancedCards ? "Hide details" : "Show details"}
          </button>
        </div>
          ...
          <section className="rounded-lg border border-emerald-100 bg-emerald-50/40 p-5 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Export</h2>
                <p className="mt-2 text-sm text-slate-600">Download this dashboard as PDF or Excel.</p>
              </div>
              {!canExport ? (
            ...
            <div className="mt-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap gap-2">
                <button
                  className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                    exportFormat === "pdf"
                      ? "bg-teal-700 text-white"
                      : "border border-emerald-200 bg-white text-slate-700 hover:border-teal-300"
                  }`}
              ...
              <button
                className="rounded-md bg-teal-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-slate-300"
