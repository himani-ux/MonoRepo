...
} from "../../../../components/safety/incident/incident-pdf-section-selector";
...
function DownloadCard({
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-900">{buttonLabel}</h2>
      <p className="mt-2 text-sm leading-6 text-slate-600">{helperText}</p>
      {disabledReason ? <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">{disabledReason}</p> : null}
      {error ? <p className="mt-3 text-sm font-medium text-rose-700">{error}</p> : null}
      <button
        className="mt-4 inline-flex min-h-11 items-center rounded-full bg-slate-900 px-5 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-400"
...
function IncidentPdfDownloadCard({ disabledReason }: { disabledReason?: string }) {
  const { id } = useParams();
  const [error, setError] = useState<string | null>(null);
  const [isDownloading, setIsDownloading] = useState(false);
  const [selectedSections, setSelectedSections] = useState<IncidentPdfSectionKey[]>(
    DEFAULT_INCIDENT_PDF_SECTION_KEYS,
  );

  async function handleDownload() {
    if (!id || selectedSections.length === 0) {
      return;
    }
    setError(null);
    setIsDownloading(true);
    try {
      downloadBlob(await safetyApi.downloadIncidentPdf(id, selectedSections));
    } catch (caught) {
      setError(getErrorMessage(caught));
    } finally {
      setIsDownloading(false);
  ...
  return (
    <section className="space-y-4">
      ...
      />
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Download incident PDF</h2>
        <p className="mt-2 text-sm leading-6 text-slate-600">Download the generated PDF.</p>
        {disabledReason ? <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">{disabledReason}</p> : null}
        {error ? <p className="mt-3 text-sm font-medium text-rose-700">{error}</p> : null}
        <button
          className="mt-4 inline-flex min-h-11 items-center rounded-full bg-slate-900 px-5 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-400"
          disabled={isDownloading || Boolean(disabledReason) || selectedSections.length === 0}
          onClick={handleDownload}
          type="button"
        >
          {isDownloading ? "Preparing export..." : "Download incident PDF"}
        </button>
      </section>
    </section>
  );
...
function isIncidentPdfExportReady(payload: unknown) {
  const incident = (payload && typeof payload === "object" ? payload : {}) as IncidentStatusPayload;
  const phase = Number(incident.current_phase ?? incident.phase ?? 0);
  const state = String(incident.state ?? "").toUpperCase();
  return phase >= 7 || state === "APPROVED" || state === "CLOSED";
}
...
function formatActionStatus(status: SafetyCorrectiveAction["status"]) {
  return status.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase());
}

function formatRecommendationTier(tier?: string | null) {
  if (!tier) {
    return "";
  }
...
function CorrectiveActionManager({
  ...
  function noteFor(action: SafetyCorrectiveAction) {
    return notes[action.id] ?? `Progress update for "${action.title}".`;
  }
    ...
    <section className="space-y-4">
      ...
      {actions.length === 0 ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-5 text-sm text-slate-600 shadow-sm">
          No actions are linked to this incident.
        </div>
      ) : null}
      {actions.map((action) => {
        const nextStatus = nextCorrectiveActionStatus[action.status];
                ...
                <div className="mt-3 grid gap-3 text-sm text-slate-600 md:grid-cols-3">
                  <p>
                    ...
                    <span className="font-semibold text-slate-900">
                      {action.recommendation_title || "Not linked"}
                    </span>
                  </p>
                  {action.recommendation_tier ? (
                    <p>
                      Type:{" "}
                      <span className="font-semibold text-slate-900">
                        {formatRecommendationTier(action.recommendation_tier)}
                      </span>
                    </p>
                  ) : null}
                  <p>Due: <span className="font-semibold text-slate-900">{action.due_date ?? "Not set"}</span></p>
                  <p>Checker: <span className="font-semibold text-slate-900">{action.verifier_user_id ?? "Not set"}</span></p>
                </div>
              </div>
  ...
  return (
    <section className="space-y-6">
      ...
      <SafetyIncidentPhase6
        fixedTier="CORRECTIVE"
        nextLabel="Continue to Preventive Action"
        nextPath={`/safety/incidents/${id}/phase-3/preventive`}
        previousLabel="Back to RCA"
      />
    </section>
  );
}

export function IncidentPreventiveActionRoute() {
  const { id } = useParams();
  return (
    <section className="space-y-6">
      <IncidentPhaseSwitcher activePhase={4} />
      <SafetyIncidentPhase6
        fixedTier="PREVENTIVE"
        nextLabel="Continue to Office Review"
        nextPath={`/safety/incidents/${id}/phase-5`}
        previousLabel="Back to Corrective Action"
        previousPath={`/safety/incidents/${id}/phase-3`}
        transitionTargetPhase={7}
      />
    </section>
  );
}

export function IncidentLessonsLearnedRoute() {
  const { id } = useParams();
  return <Navigate replace to={`/safety/incidents/${id}/phase-5`} />;
}

export function IncidentPhase4Route({ activeTab = "paper" }: { activeTab?: SafetyIncidentPhase3TabKey }) {
  return (
    <section className="space-y-6">
      <IncidentPhaseSwitcher activePhase={6} />
      <SafetyIncidentPhase3 activeTab={activeTab} routeBase="phase-4" />
    </section>
  );
}
...
export function IncidentPhase6Route() {
  return (
    <section className="space-y-6">
      <IncidentPhaseSwitcher activePhase={3} />
      <SafetyIncidentPhase6 fixedTier="CORRECTIVE" />
    </section>
  );
}
...
export function IncidentPhase8Route() {
  return (
    <section className="space-y-6">
      <IncidentPhaseSwitcher activePhase={8} />
      <SafetyIncidentPhase8 />
    </section>
  );
}
    ...
    <section className="space-y-6">
      <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              Finish all actions before closing the incident.
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <Link
            className="inline-flex min-h-11 items-center rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700"
            to={`/safety/incidents/${id}/phase-6`}
          >
...
export function IncidentPdfRoute() {
  return (
    ...
    >
      {({ payload }) => (
        <IncidentPdfDownloadCard
          disabledReason={
            isIncidentPdfExportReady(payload)
              ? undefined
              : "Incident PDF is available after Office Review acceptance."
          }
        />
      )}
    </IncidentOfficeReadRoute>
...
export function IncidentMscMepc3Route() {
  return (
    ...
    >
      {() => (
        <DownloadCard
          buttonLabel="Download IMO report"
          downloader={safetyApi.downloadIncidentMscMepc3}
          helperText="Download the IMO report."
        />
      )}
    </IncidentOfficeReadRoute>
  );
