...
function getVesselDashboardIdentifier(auth: CertAuthContext): string {
...
function CertsLandingStub() {
  ...
  const vesselDashboardIdentifier = getVesselDashboardIdentifier(auth);
      ...
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-4 py-6">
        <Card>
          <CardContent className="space-y-4 p-6 text-center">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-md bg-primary-50 text-primary-600">
              <FileCheck2 className="h-6 w-6" aria-hidden="true" />
            </div>
            <h1 className="text-xl font-semibold text-neutral-900">
              Certs module coming soon
            </h1>
...
function CertVesselDashboardPage({ imo }: { imo: string }) {
  ...
  const canPrint = useCertsPermission(FORM_IDS.CERTS_PRINT_EXPORT, PROCESS_IDS.CERTS_PRINT);
  const canShareBundle = useCertsPermission(FORM_IDS.CERTS_PRINT_EXPORT, PROCESS_IDS.CERTS_EXPORT_BUNDLE);
  const canUploadSnapshot = useCertsPermission(FORM_IDS.CERTS_RECONCILIATION, PROCESS_IDS.CERTS_CREATE);
  const auth = useAuth();
  ...
  const canUploadClassSnapshot = canUploadSnapshot && !['VESSEL', 'MASTER', 'CHIEF OFFICER', 'CHIEF ENGINEER', 'SECOND ENGINEER'].some((marker) => role.includes(marker));
    ...
    <RootLayout>
      ...
      <div className="space-y-4 p-4">
        <CertVesselHeader
          ...
          canUploadSnapshot={canUploadClassSnapshot}
        />
...
function CertVesselSpecialBanners({ data, imo }: { data: CertVesselDashboardResponse; imo: string }) {
  ...
  if (data.mandatoryCoverage.percent < 100) {
    ...
    banners.push({
      key: 'coverage',
      variant: data.mandatoryCoverage.overrideActive ? 'warning' : 'destructive',
      text: data.mandatoryCoverage.overrideActive
        ? `Mandatory coverage override active: ${data.mandatoryCoverage.overrideReason}`
        : `Mandatory coverage is ${data.mandatoryCoverage.percent}%; ${missingCount} mandatory cert${missingCount === 1 ? '' : 's'} still missing.`,
    });
  }
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="grid gap-4 xl:grid-cols-[1.05fr_1fr_1fr]">
          ...
          <CertTrackedItemWorkflowPanel
            item={item}
            imo={imo}
            canDirectEdit={canDirectEditItem}
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        />
        {(canSubmitCurrent || (canMasterDecision && (canApprove || canReject)) || canDirectEdit) ? (
        ...
        <CertChangeHistory entries={item.changeHistory} />
      </CardContent>
    </Card>
  );
}
...
function CertChangeHistory({ entries }: { entries: CertTrackedItemDetail['changeHistory'] }) {
  return (
    <div className="space-y-2">
      <h2 className="text-sm font-semibold uppercase text-neutral-500">Field history</h2>
      {entries.length === 0 ? (
        <p className="text-sm text-neutral-600">No field history recorded.</p>
      ) : (
        <div className="space-y-2">
          {entries.map((entry) => (
            <div key={entry.id} className="rounded-md border border-neutral-200 p-3 text-sm">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="font-medium text-neutral-900">{entry.fieldName}</span>
                <Badge variant="secondary">v{entry.versionAfter}</Badge>
              </div>
              <p className="mt-1 text-neutral-600">
                {formatChangeHistoryValue(entry.fieldName, entry.oldValue)} to {formatChangeHistoryValue(entry.fieldName, entry.newValue)}
              </p>
              <p className="mt-1 text-xs text-neutral-500">
                {entry.sourceModule} - {formatPrincipalLabel(entry.changedByDisplay, entry.changedBy, undefined, 'system')} - {formatDateTime(entry.changedAt)}
              </p>
            </div>
          ))}
        </div>
        ...
        <tbody className="divide-y divide-neutral-100">
          {orderedRows.map((row) => {
            ...
            <tr key={row.id} className="cursor-pointer hover:bg-neutral-50" onClick={() => navigate(ROUTES.CERTS_CATALOG_DETAIL(row.id))}>
              ...
              <td className="whitespace-nowrap px-3 py-3 text-neutral-700">{row.cadenceMonths ?? 'Permanent'}</td>
              <td className="whitespace-nowrap px-3 py-3 text-neutral-700">{row.submissionScope}</td>
...
function formatChangeHistoryValue(fieldName: string | null | undefined, value: unknown): string {
  const formatted = formatUnknown(value);
  if (!isUuidLike(formatted)) return formatted;

  const normalizedField = String(fieldName ?? '').toLowerCase();
  if (normalizedField.includes('vessel')) return 'Vessel linked';
  if (normalizedField.includes('catalog')) return 'Catalog row linked';
  if (normalizedField.includes('pdf') || normalizedField.includes('attachment') || normalizedField.includes('blob')) return 'PDF attached';
  if (normalizedField.includes('parent')) return 'Parent certificate linked';
  if (normalizedField.includes('supersedes')) return 'Superseded certificate linked';
  if (normalizedField.includes('submitted_by')) return 'Submitter recorded';
  if (normalizedField.includes('approved_by')) return 'Approver recorded';
  if (normalizedField.includes('rejected_by')) return 'Rejector recorded';
