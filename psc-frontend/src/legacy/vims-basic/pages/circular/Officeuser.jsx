...
import {
  ...
  CardTitle,
  CardContent,
} from "../../components/circular/ui/card";
...
const Admin = ({ onNotificationSubmit }) => {
    ...
    const fetchDraftForEditing = async () => {
      try {
        const response = await fetch(
          `http://localhost:8001/api/circular/api/draft/${normalizedDraftSrNo}/`,
        );
  ...
  const [
    ...
    setOldNotificationSecondSubCatNames,
  ] = useState([]);
    ...
    const fetchOptions = async () => {
      try {
        ...
        const docRes = await fetch(
          "http://localhost:8001/api/circular/api/document-types/",
        );
        ...
        const deptRes = await fetch(
          "http://localhost:8001/api/circular/api/departments/",
        );
        ...
        if (Array.isArray(deptData)) {
          deptData.forEach((item) => {
            if (item && item[0] && item[1]) {
              // Ensure the item array has both ID (index 0) and Name (index 1)
        ...
        const prioRes = await fetch(
          "http://localhost:8001/api/circular/api/priorities/",
        );
        ...
        if (Array.isArray(prioData)) {
          ...
          const normalizedPrios = prioData
            .map((item) => item[1])
            .filter(Boolean); // Extract the name (second element)
        ...
        const subCatRes = await fetch(
          "http://localhost:8001/api/circular/api/sub-categories/",
        );
        ...
        if (Array.isArray(subCatData)) {
          ...
          subCatData.forEach((item) => {
            if (item && item.id && item.name) {
              // Assuming item = {id: "uuid", name: "Flag", ...}
              subCatMap[item.name] = item.id; // name -> id
            }
          });
          setSubCatToIdMap(subCatMap); // Store the map in state

          const normalizedSubCats = subCatData
            .map((item) => item.name)
            .filter(Boolean); // Extract the name
      ...
      try {
        const [typesRes, catsRes, subCatsRes, secondSubCatsRes, prioritiesRes] =
          await Promise.all([
            ...
            fetch(
              "http://localhost:8001/api/circular/api/second-sub-categories/",
        ...
        console.log("Fetched lookup data:", {
          types,
          cats,
          subCats,
          secondSubCats,
    ...
    const fetchSubmittedRequests = async () => {
      try {
        const res = await fetch(
          "http://localhost:8001/api/circular/api/submitted/",
        );
  ...
  useEffect(() => {
    ...
    const fetchSecondSub = async () => {
      try {
        ...
        const targetDeptUuid =
          selectedMainOption === "seq"
        ...
        if (Array.isArray(allData)) {
          ...
          const filteredData = allData.filter((item) => {
            ...
            return (
              item.department_id.toLowerCase() === targetDeptUuid.toLowerCase()
            );
          });
          ...
          filteredData.forEach((item) => {
            if (item && item.id && item.name) {
              // Assuming item = {id: "uuid", name: "Flag", department_id: "uuid", ...}
  ...
  useEffect(() => {
    const fetchVessels = async () => {
      if (showVesselPopup || showApprovalPopup) {
        ...
        try {
          const response = await fetch(
            "http://localhost:8001/api/circular/api/vessels/",
          ); // Use your vessel API endpoint
          if (!response.ok) {
            throw new Error(
              `Failed to fetch vessels: ${response.status} ${response.statusText}`,
            );
          }
  ...
  useEffect(() => {
    const fetchAllRanks = async () => {
      ...
      if (showRankPopup || hasApprovalDeliveryContext) {
        ...
        try {
          const response = await fetch(
            "http://localhost:8001/api/circular/api/ranks/",
          ); // Use the new endpoint
          if (!response.ok) {
            throw new Error(
              `Failed to fetch all ranks: ${response.status} ${response.statusText}`,
            );
          }
          const data = await response.json();
  ...
  useEffect(() => {
    ...
    if (storedSrNo) {
      console.log(
        "Component mounted: Found supersedingNotificationId in localStorage:",
        storedSrNo,
      );
      setSupersedingNotificationSrNo(storedSrNo); // Set the state variable
    } else {
      console.log(
        "Component mounted: No supersedingNotificationId found in localStorage.",
      );
  ...
  const handleEditPendingNotification = async (srNoToEdit) => {
    ...
    console.log(
      "handleEditPendingNotification: Fetching notification details for SR No:",
      srNoToEdit,
    );
    try {
      const response = await fetch(
        `http://localhost:8001/api/circular/api/submitted/${srNoToEdit}/`,
      ); // Use your endpoint to get single notification details by SR No
      if (!response.ok) {
        throw new Error(
          `Failed to fetch notification details: ${response.status} ${response.statusText}`,
        );
      }
      const notificationDetails = await response.json();
      console.log(
        "handleEditPendingNotification: Retrieved notification details:",
        notificationDetails,
      );
      ...
      console.log(
        "handleEditPendingNotification: Navigating to main form page to load pending notification data for editing.",
      );
      navigate("/"); // Navigate to the main form page (assuming it's '/') // Adjust path if necessary
    } catch (detailsError) {
      console.error(
        "handleEditPendingNotification: Error fetching notification details:",
        detailsError,
      );
      alert(
  ...
  const handlePublishClick = (e) => {
    ...
    console.log(
      "Publish button clicked, triggering notification creation flow.",
    );
  ...
  // Example function to open the vessel selection popup for publishing
  const handleVesselSelectionForPublishing = () => {
    // Clear any previously selected vessels if needed
    // setSelectedVesselIds(new Set()); // Optional: reset selection

    // Set the state to show the vessel popup
    setShowVesselPopup(true);

    // Optionally, clear the approvingNotificationSrNo if this is for a new publish action
    // setApprovingNotificationSrNo(null); // Or however you manage the context for this action

    console.log("Opening vessel selection popup for publishing.");
  };
  ...
  const handleSelectAllRanksChange = () => {
    console.log(
      "handleSelectAllRanksChange: Select All clicked. Current selected count:",
      selectedRankIds.size,
      "Total ranks:",
      allRanks.length,
    );
  ...
  const handleSelectAllDeckRanksChange = () => {
    console.log(
      "handleSelectAllDeckRanksChange: Select All DECK clicked. Current selected count:",
      selectedRankIds.size,
      "Total DECK ranks:",
      seqRanks.length,
    );
    ...
    if (
      seqRanks.length > 0 &&
      seqRanks.every((r) => selectedRankIds.has(r.id))
    ) {
  ...
  const handleSelectAllEngineRanksChange = () => {
    console.log(
      "handleSelectAllEngineRanksChange: Select All ENGINE clicked. Current selected count:",
      selectedRankIds.size,
      "Total ENGINE ranks:",
      technicalRanks.length,
    );
    ...
    if (
      ...
      technicalRanks.every((r) => selectedRankIds.has(r.id))
    ) {
  ...
  const submitApprovalOrRejection = async (
    sr_no,
    action,
    comment,
    vesselIds = null,
  ...
  const handleConfirmVesselSelectionForApproval = async () => {
    ...
    console.log(
      "handleConfirmVesselSelectionForApproval: Selected vessel IDs:",
      vesselIdsArray,
    );

    // Get the original notification SR No from localStorage (set in handleApproveReject)
    const approvingNotificationSrNo = localStorage.getItem(
      "approvingNotificationSrNo",
    );
    console.log(
      "handleConfirmVesselSelectionForApproval: approvingNotificationSrNo found:",
      approvingNotificationSrNo,
    );
  ...
  const handleConfirmApprovalWithComment = async () => {
    ...
    if (!notificationSrNoForComment) {
      console.error(
        "handleConfirmApprovalWithComment: Missing notification SR No in state.",
      );
    ...
    if (
      !Array.isArray(vesselIdsForComment) ||
      vesselIdsForComment.length === 0
    ) {
      console.error(
        "handleConfirmApprovalWithComment: No vessel IDs available for approval.",
      );
      alert(
        "No vessels selected for approval. Please select at least one vessel.",
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (!emailResponse.ok) {
          ...
          alert(
            `Approved, but failed to send emails: ${emailResult.error || "Unknown error"}`,
          );
        } else {
          console.log(
            "handleConfirmApprovalWithComment: Emails sent:",
            emailResult,
          );
        }
      } catch (emailErr) {
    ...
    try {
      ...
      if (rankIdsForApproval.length > 0) {
        const rankResponse = await fetch(
          `http://localhost:8001/api/circular/api/notifications/${notificationSrNo}/link-ranks/`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const handleConfirmRankSelection = async () => {
    ...
    const approvingNotificationId = localStorage.getItem(
      "approvingNotificationId",
    ); // This might be null if you are not storing it
    const approvingNotificationSrNo = localStorage.getItem(
      "approvingNotificationSrNo",
    );

    if (!approvingNotificationSrNo) {
      // You only need the SR No for the rank selection logic
      console.error(
        "handleConfirmRankSelection: No notification SR No found in localStorage.",
      );
      alert("An error occurred. Please try again.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8001/api/circular/api/notifications/${approvingNotificationSrNo}/link-ranks/`,
        {
          // Use a new endpoint
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        ...
        console.log(
          "handleConfirmRankSelection: Cleared approvingNotificationSrNo and approvingNotificationDept from localStorage.",
        );
        ...
        console.error(
          "handleConfirmRankSelection: Rank link response error:",
          result,
        );
      }
    } catch (rankErr) {
      console.error(
        "handleConfirmRankSelection: Network error during rank selection confirmation:",
        rankErr,
      );
  ...
  const handleFileChange = (e) => {
    ...
    if (validFiles.length > 0) {
      ...
      console.log(
        "handleFileChange: Added valid PDF files to state:",
        filesToAdd,
      );

      if (skippedFiles.length > 0) {
        alert(
          `Maximum ${MAX_CIRCULAR_ATTACHMENT_FILES} PDFs allowed. These files were not added: ${skippedFiles.join(", ")}`,
        );
      }
    } else {
  ...
  const buildDraftFormData = () => {
    ...
    if (!selectedTypeId) {
      alert(
        `Document type ID not found for selection '${selectedType}'. Please try again.`,
      );
      console.error(
        "Missing ID for selected type:",
        selectedType,
        "Map:",
        typeToIdMap,
      );
    ...
    if (!selectedDeptId) {
      alert(
        `Department ID not found for selection '${deptNameForMap}'. Please try again.`,
      );
      console.error(
        "Missing ID for department key:",
        deptNameForMap,
        "Map:",
        deptToIdMap,
      );
    ...
    if (!selectedPriorityId) {
      alert(
        `Priority ID not found for selection '${selectedSeverity}'. Please try again.`,
      );
      console.error(
        "Missing ID for selected priority key:",
        selectedSeverity,
        "Map:",
        priorityToIdMap,
      );
  ...
  useEffect(() => {
    if (draftPrefillData) {
      try {
        ...
        if (
          draft.office_instructions !== undefined &&
          draft.office_instructions !== null
        ) {
          setBody(draft.office_instructions);
          console.log(
            "Set body from office_instructions:",
            draft.office_instructions,
          );
        } else {
          console.log(
            "No office_instructions field found in draft data or it was empty, setting body to empty string",
          );
          setBody("");
        }
        ...
        if (
          draft.selectedMainOptionForPreFill ||
          (draft.dept !== undefined && draft.dept !== null)
        ) {
        ...
        if (draft.selectedCategoryForPreFill || draft.category) {
          const mappedCategory =
            draft.selectedCategoryForPreFill ||
            String(draft.category || "").toLowerCase();
        ...
        if (draft.selectedSub2ForPreFill || draft.second_sub_category) {
          ...
          console.log(
            "Set selectedSub2:",
            Array.from(secondSubCatSet),
            "from second_sub_category:",
            draft.second_sub_category,
          );
        }
  ...
  const handleConfirmPublish = async () => {
    ...
    console.log(
      "handleConfirmPublish: Confirm clicked. Checking for approval context...",
    );
    ...
    console.log(
      "handleConfirmPublish: approvingNotificationSrNo (Serial Number) found:",
      approvingNotificationSrNo,
    );
    // --- END NEW ---

    if (approvingNotificationSrNo) {
      // Check for SR No instead of ID
      // --- CONFIRM APPROVAL FLOW (Vessel Selection Popup during Approval) ---
      console.log(
        "handleConfirmPublish: Executing vessel selection confirmation for approval of notification SR No:",
        approvingNotificationSrNo,
      );
      ...
      console.log(
        "handleConfirmPublish: Selected vessel IDs for approval:",
        selectedVesselIdsArray,
      );

      if (selectedVesselIdsArray.length === 0) {
        alert(
          "Please select at least oneEE vessel before confirming approval.",
        );
      ...
      const userChoice = window.confirm(
        `Do you want to add a comment for approval?`,
      );
      if (userChoice) {
        ...
        console.log(
          "handleConfirmPublish: User entered comment for approval:",
          comment,
        );
      } else {
      ...
      console.log(
        "handleConfirmPublish: Prepared payload for approval update:",
        payload,
      );
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8001/api/circular/api/notifications/${approvingNotificationSrNo}/update-status/`,
          {
            // Use SR No here
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (response.ok) {
          ...
          console.log(
            "handleConfirmPublish: Cleared approvingNotificationSrNo from localStorage.",
          );
          ...
          console.log(
            "handleConfirmPublish: Vessel selection confirmed. Now showing rank selection popup.",
          );
          ...
          try {
            ...
            const detailsResponse = await fetch(
              `http://localhost:8001/api/circular/api/submitted/${approvingNotificationSrNo}/`,
            ); // Fetch using SR No
            if (!detailsResponse.ok) {
              throw new Error(
                `Failed to fetch notification details: ${detailsResponse.status} ${detailsResponse.statusText}`,
              );
            }
            const notificationDetails = await detailsResponse.json();
            console.log(
              "handleConfirmPublish: Retrieved notification details for rank fetch:",
              notificationDetails,
            );
            ...
            if (deptNameForRanks === "Unknown") {
              console.warn(
                "handleConfirmPublish: Could not determine department for notification SR No",
                approvingNotificationSrNo,
            ...
            console.log(
              "handleConfirmPublish: Stored approvingNotificationDept in localStorage:",
              deptNameForRanks,
            );
            ...
            setShowRankPopup(true);
            console.log(
              "handleConfirmPublish: Rank selection popup displayed.",
            );
          } catch (detailsError) {
            console.error(
              "handleConfirmPublish: Error fetching notification details for rank selection popup:",
              detailsError,
            );
            alert(
              `Approved, but could not load ranks for selection: ${detailsError.message}`,
            );
          ...
          console.error(
            "handleConfirmPublish: Approval response error:",
            result,
          );
        }
      } catch (err) {
        console.error(
          "handleConfirmPublish: Network error during approval:",
          err,
        );
        alert("Network error");
      }
      ...
      if (!selectedCategory) {
        // selectedCategory should hold the NAME from the dropdown
        alert("Please select a category: Internal or External.");
        return;
      }
      ...
      if (selectedCategory === "internal" && selectedSub2.size === 0) {
        alert(
          "Please select at least one option under the second subcategory.",
        );
        return;
      }
      // --- End Validation ---
      ...
      const selectedTypeId = resolveCircularMappedId(
        selectedType,
        typeToIdMap,
      );
      ...
      selectedSub1.forEach((name) => {
        ...
        if (id) {
          ...
          console.warn(
            "Could not find ID for sub-category name:",
            name,
            "Map:",
            subCatToIdMap,
          );
      ...
      Array.from(selectedSub2).forEach((name) => {
        ...
        if (id) {
          ...
          console.warn(
            "Could not find ID for second sub-category name:",
            name,
            "Map:",
            secondSubCatToIdMap,
          );
      ...
      try {
        ...
        const response = await fetch(
          "http://localhost:8001/api/circular/api/notifications/",
          {
            method: "POST",
        ...
        if (response.ok) {
          ...
          if (newNotificationSrNo) {
            ...
            alert(
              "Notification was saved, but an error occurred (SR No not returned).",
            );
          }
  ...
  const handleSelectAllChange = () => {
    console.log(
      "Select All checkbox clicked. Current selected count:",
      selectedVesselIds.size,
      "Total vessels:",
      vessels.length,
    );
  ...
  const handleSubmit = async (e) => {
    ...
    const supersedingOldSrNo = localStorage.getItem(
      "supersedingNotificationId",
    ); // Use consistent key
    console.log(
      ".handleSubmit: Checking for superseding notification ID (OLD SR No):",
      supersedingOldSrNo,
    );
    // --- END NEW ---

    console.log(
      "handleSubmit: editingDraftId (database ID) found:",
      editingDraftId,
    );
    ...
    console.log(
      "handleSubmit: Office User submitting with initial publish_status:",
    ...
    const segments = trimmedHashtags
      .split(/\s+/)
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (!segment.startsWith("#")) {
        ...
        alert(
          `Invalid hashtag format: '${segment}'. Each hashtag must start with '#'.`,
        );
      ...
      if (hashCount !== 1) {
        ...
        alert(
          `Invalid hashtag format: '${segment}'. A hashtag must start with a single '#'.`,
        );
        break;
      }
      ...
      if (segment.length <= 1) {
        ...
        alert(
          `Invalid hashtag format: '${segment}'. A hashtag must have content after '#'.`,
        );
        break;
      }
    ...
    // --- NEW: Use ID Maps (Plain Objects) to get UUIDs ---
    ...
    if (!selectedTypeId) {
      ...
      console.error(
        "handleSubmit: Missing ID for selected type key:",
        selectedType,
        "Map:",
        typeToIdMap,
      );
      return; // Stop execution if ID not found
    }
    ...
    if (!departmentIdToSend) {
      ...
      console.error(
        "handleSubmit: Missing ID for department key:",
        deptNameForMap,
        "Map:",
        deptToIdMap,
      );
    ...
    if (!selectedPriorityId) {
      ...
      console.error(
        "handleSubmit: Missing ID for selected priority key:",
        selectedSeverity,
        "Map:",
        priorityToIdMap,
      );
    ...
    if (supersedingOldSrNo) {
      console.log(
        "handleSubmit: Adding supersede data for OLD SR No:",
        supersedingOldSrNo,
      );
    ...
    Array.from(selectedSub1).forEach((name) => {
      ...
      if (id) {
        ...
        // Decide how to handle missing IDs (e.g., skip, show alert, return)
      }
    });
    ...
    try {
      ...
      if (draftUpdateUrl) {
        ...
        console.log(
          `Sending request to update endpoint: ${draftUpdateUrl}`,
        );
        ...
        if (response.ok) {
          ...
          if (supersedingOldSrNo) {
            // Check if the ID was present *before* the request was sent
            localStorage.removeItem("supersedingNotificationId"); // Use consistent key
            console.log(
              "handleSubmit: Cleared supersedingNotificationId from localStorage after successful draft update.",
            );
          } else {
            console.log(
              "handleSubmit: No supersedingNotificationId was present initially, no need to clear.",
            );
          }
          // --- END NEW ---
        ...
        console.log(
          "Sending request to create endpoint: /api/circular/api/notifications/",
        );
        ...
        if (response.ok) {
          ...
          // Clear the Supersede Flag from LocalStorage Regardless of Out Come
          if (supersedingOldSrNo) {
            localStorage.removeItem("supersedingNotificationId"); // Use consistent key
            console.log(
              "handleSubmit: Cleared supersedingNotificationId from localStorage.",
            );
          }
          // --- END NEW ---
          ...
          console.log(
            "=== Starting Crew List Display Process (New Submission - Pending) ===",
          );
          const deptNameForShipside =
            selectedMainOption === "seq" ? "Deck" : "Engine";
          ...
          try {
            ...
            if (!crewResponse.ok) {
              throw new Error(
                `Failed to fetch crews: ${crewResponse.status} ${crewResponse.statusText}`,
              );
            }
