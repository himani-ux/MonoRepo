...
const DraftNotifications = ({ currentUser }) => {
    ...
    const fetchUserDrafts = async (order = sortDirection, criteria = sortCriteria) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8001/api/circular/api/user-drafts/?${queryParams}`);
    ...
    const handleDeleteClick = async (draftId, srNo, displaySrNo) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8001/api/circular/api/drafts/${draftId}/delete/`, {
