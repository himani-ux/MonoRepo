...
export default function ChiefDashboard() {
  ...
  const handleReportPeriod = async (period) => {
    ...
    try {
      const url = `http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&status=Approved&is_deleted=false`;
...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchCSRF = async () => {
      const response = await fetch("http://localhost:8001/api/orb/api/csrf/", {
  ...
  const handleCustomDateRange = async (from, to) => {
    ...
    try {
      ...
      const url = `http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&status=Approved&is_deleted=false&from_date=${from}&to_date=${to}`;
  ...
  try {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8001/api/get_last_page_number/?vessel_id=${vesselId}`);
      const data = await res.json();
    ...
    try {
      ...
      const ipResponse = await fetch('http://localhost:8001/api/orb/api/get-internal-ip/'); // Use your backend's URL
      if (!ipResponse.ok) {
    ...
    try {
      ...
      reader.onloadend = async function () {
        ...
        const metadataResponse = await fetch("http://localhost:8001/api/orb/api/save-pdf-metadata/", {
          method: "POST",
          headers: {
  ...
  const updatePrintStatusInDB = async (ip, timestamp) => {
    ...
    try {
      const response = await fetch("http://localhost:8001/api/orb/api/update-print-status/", { // this is the backend endpoint(API)
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const doApprove = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8001/api/orb/api/operations/${id}/approve/`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
  ...
  const doIncorrect = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8001/api/orb/api/operations/${id}/reject/`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
