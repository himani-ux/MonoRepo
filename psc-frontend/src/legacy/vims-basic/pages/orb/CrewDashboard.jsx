...
export default function CrewDashboard({ isChiefMode = false, onSubmit }) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=false&status=Pending`)
      .then((r) => r.json())
      .then((data) => {
...
useEffect(() => {
  async function fetchCodes() {
    try {
      const res = await fetch("http://localhost:8001/api/orb/api/codes/");
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8001/api/orb/api/tanks-for-orb/?vessel_id=${vesselParam}&orb_code=${formData.code}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const refreshEntries = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=false&status=Pending`);
      if (!res.ok) {
      ...
      const approvedRes = await fetch(`http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=false&status=Approved`);
      ...
      const rejectedRes = await fetch(`http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=false&status=Rejected`);
      ...
      const deletedRes = await fetch(`http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=true`);
  ...
  const handleValidate = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=false`);
      const data = await res.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const latestResponse = await fetch(`http://localhost:8001/api/orb/api/latest-entry-date/?vessel_id=${vesselId}`);
    ...
    try {
      ...
      const allRawEntriesResponse = await fetch(
        `http://localhost:8001/api/orb/api/operations/?vessel_id=${vesselId}&is_deleted=false`
      );
    ...
    if (editingEntryId) {
      ...
      try {
        ...
        const updateResponse = await fetch(`http://localhost:8001/api/orb/api/operations/${editingEntryId}/update-group/`, {
          method: 'PATCH',
          headers: {
    ...
    if (formData.code === 'I' && pendingFEntry) {
      ...
      try {
        //  Save Code F
        await fetch("http://localhost:8001/api/orb/api/operations/", {
          method: "POST",
    ...
    try {
      const response = await fetch("http://localhost:8001/api/orb/api/operations/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8001/api/orb/api/operations/${id}/`, { // Correct endpoint
