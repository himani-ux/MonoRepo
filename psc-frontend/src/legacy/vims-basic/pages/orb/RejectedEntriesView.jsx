...
import { Panel, Button, Card, Stack } from "../../components/orb/OrbUI"; // Adjust path as needed
...
const RejectedEntriesView = () => {
  ...
  if (loading) {
    return <div className="rejected-entries-view">Loading rejected entries...</div>;
  }

  // Render error state
  if (error) {
    return <div className="rejected-entries-view">Error: {error}</div>;
  }

  // Render the table with entries using the specific format
  return (
    
    <div className="rejected-entries-view orb-theme"> {/* Apply your theme class if needed */}
      <Card>
        <div className="p-card-title">
          <h2>Rejected ORB Entries</h2>
        </div>
        <div className="p-card-content">
          ...
          <div style={{
            ...
            <span style={{ fontSize: '1.1em' }}>🛳️</span> {/* Or use an SVG icon */}
            <span>
          ...
          <div style={{ overflowX: 'auto', width: '100%', margin: '1rem 0' }}>
            <table className="orb-table" style={{ minWidth: '800px', width: '100%' }}>
              ...
              <tbody>
                {entries.length === 0 ? (
                  <tr>
                    <td colSpan="7" style={{ textAlign: 'center' }}>No rejected entries found.</td>
                  </tr>
                ) : (
