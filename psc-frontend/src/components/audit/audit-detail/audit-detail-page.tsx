...
const REQUIRED_SCORECARD_ROW_COUNT = 14;
...
function auditStatusVariant(status: string) {
  ...
  if (status === 'IN_PROGRESS' || status === 'VESSEL_ACKNOWLEDGED')
    return 'warning';
  return 'secondary';
}

function requiredScorecardRows(rows: AuditScorecardRow[]): AuditScorecardRow[] {
  return [...rows]
    .sort(
      (left, right) =>
        left.sequence_no - right.sequence_no ||
        left.area_code.localeCompare(right.area_code)
    )
    .slice(0, REQUIRED_SCORECARD_ROW_COUNT);
}
...
export function AuditDetailPage({ auditId }: AuditDetailPageProps) {
  ...
  const [submitGateErrors, setSubmitGateErrors] =
    useState<SubmitGateErrors | null>(null);
  ...
  useEffect(() => {
    ...
    setScorecardRows(requiredScorecardRows(audit.scorecard));
  }, [audit]);
  ...
  if (audit?.audit_classification === 'EXTERNAL') {
    return <Navigate to={`/audit/external/${audit.id || auditId}`} replace />;
  }
  ...
  const setField = (
    fieldName: keyof AuditDetailEditableFields,
    value: string
  ) => {
    setFields((current) =>
      current ? { ...current, [fieldName]: value } : current
    );
  };
  ...
  const saveDetail = async () => {
    try {
      await updateDetail.mutateAsync({
        ...fields,
        opening_meeting_at: fromInputDateTime(
          String(fields.opening_meeting_at || '')
        ),
        closing_meeting_at: fromInputDateTime(
          String(fields.closing_meeting_at || '')
        ),
      });
      ...
      toast({
        title:
          result.status === 'ALREADY_LINKED'
            ? 'Circular already linked'
            : 'Circular draft created',
      });
  ...
  const canAcknowledgeAudit = effectivePermissions.has(
  ...
  return (
    <RootLayout>
      ...
      <div className="space-y-4 p-4">
        <AuditHeader audit={audit} vesselOptions={vesselQuery.data ?? []} />
        {submitGateErrors && (
          <SubmitGateFailureBanner gates={submitGateErrors} />
        )}
          ...
          <CardContent className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-medium text-neutral-800">
                Current status: {getStatusLabel(audit.status)}
              </p>
              <p className="text-sm text-neutral-500">
                Submit runs the four D-071 gates; acknowledgement starts the
                vessel-received status chain.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {audit.status === 'REPORT_FINALIZED' && canAcknowledgeAudit && (
                <Button
                  ...
                  disabled={acknowledgeAudit.isPending}
                >
                  {acknowledgeAudit.isPending ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-2">
              <div
                className="space-y-2"
                data-eid="MOCKUP-AUDIT-02:detail.scope"
              >
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setField('audit_scope', event.target.value)
                  }
                  rows={4}
                />
              </div>
              <div className="space-y-2" data-eid="MOCKUP-AUDIT-02:detail.tor">
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setField('terms_of_reference', event.target.value)
                  }
                  rows={4}
                />
              </div>
              <div
                className="space-y-2"
                data-eid="MOCKUP-AUDIT-02:detail.opening_meeting"
              >
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setField('opening_meeting_at', event.target.value)
                  }
                />
              </div>
              <div
                className="space-y-2"
                data-eid="MOCKUP-AUDIT-02:detail.closing_meeting"
              >
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setField('closing_meeting_at', event.target.value)
                  }
                />
              </div>
              <VerifySelect
                id="prev_internal_ca_verified"
                label="Prev Internal CAR Verified"
                value={fields.prev_internal_ca_verified || ''}
                onChange={(value) =>
                  setField('prev_internal_ca_verified', value)
                }
                eid="MOCKUP-AUDIT-02:detail.prev_internal_ca"
              />
              <VerifySelect
                id="prev_external_ca_verified"
                label="Prev External CAR Verified"
                value={fields.prev_external_ca_verified || ''}
                onChange={(value) =>
                  setField('prev_external_ca_verified', value)
                }
            ...
            <div className="flex justify-end">
              <Button
                onClick={saveDetail}
                disabled={updateDetail.isPending || !canEditAudit}
              >
        ...
        <Card data-eid="MOCKUP-AUDIT-02:detail.scorecard_grid">
          <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="text-sm text-neutral-500">
              {completeScorecardCount}/{REQUIRED_SCORECARD_ROW_COUNT} populated
            </span>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-end">
              <Button
                ...
                disabled={updateScorecard.isPending || !canConductAudit}
              >
                ...
                <tbody className="divide-y divide-neutral-100 bg-white">
                  {audit.findings.length ? (
                    audit.findings.map((finding) => (
                      <tr key={finding.id}>
                        <td className="px-3 py-2">{formatEnumLabel(finding.finding_type)}</td>
                        <td className="px-3 py-2 text-neutral-800">
                          {finding.description}
                        </td>
                        <td className="px-3 py-2">
                          {finding.clause_ref_text ||
                        ...
                        <td className="px-3 py-2">
                          ...
                          >
                            {formatEnumLabel(finding.priority)}
                          </Badge>
                        </td>
                        ...
                        <td className="px-3 py-2">
                          {getStatusLabel(finding.car_status) || '-'}
                        </td>
                        <td className="px-3 py-2">
                          {finding.finding_type === 'NC' ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                              <Button asChild variant="outline" size="sm">
                                <Link
                                  to={`/audit/findings/${finding.id}/obs/wizard`}
                                >
                                  <ClipboardList className="mr-2 h-4 w-4" />
                                  Observation Closure
                                </Link>
                        ...
                        <td className="px-3 py-2">
                          {finding.linked_circular_id ? (
                            ...
                            finding.is_fleetwide_relevance ? (
                            <Button
                              type="button"
                              variant="outline"
                              size="sm"
                              ...
                              onClick={() =>
                                issueCircularForFinding(finding.id)
                              }
                            >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div
              className="space-y-2"
              data-eid="MOCKUP-AUDIT-02:detail.summary"
            >
              ...
              <Textarea
                ...
                onChange={(event) =>
                  setField('audit_summary', event.target.value)
                }
                rows={5}
              />
              <p
                className={cn(
                  'text-xs',
                  fields.audit_summary.length >= 100
                    ? 'text-success-700'
                    : 'text-neutral-500'
                )}
              >
            ...
            <div
              className="space-y-2"
              data-eid="MOCKUP-AUDIT-02:detail.equipment_tested"
            >
              <Label htmlFor="equipment_tested">
                Equipment Tested Successfully
              </Label>
              <Textarea
                id="equipment_tested"
                value={fields.equipment_tested}
                onChange={(event) =>
                  setField('equipment_tested', event.target.value)
                }
                rows={4}
              />
            </div>
...
function AuditHeader({
  ...
  const entity = auditHeaderEntity(audit, vesselOptions);
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h1
              ...
              data-eid="MOCKUP-AUDIT-02:detail_header.entity"
            >
              {entity}
            </h1>
          ...
          <p className="mt-1 text-sm text-neutral-500">
            {formatEnumLabel(audit.audit_classification)} - {audit.standards.join('/')} - Lead
            Auditor {audit.lead_auditor_name}
          </p>
          <p
            className="mt-1 text-sm text-neutral-500"
            data-eid="MOCKUP-AUDIT-02:detail.auditors"
          >
            Team:{' '}
            {audit.team_members
              .map((member) => member.member_name)
              .join(', ') || 'Lead auditor only'}
          </p>
        </div>
        <HeaderDatum
          label="Location / Port"
          value={audit.inspection.port_place}
          eid="MOCKUP-AUDIT-02:detail_header.port"
        />
        <HeaderDatum
          label="Audit Dates"
          value={dates}
          eid="MOCKUP-AUDIT-02:detail_header.dates"
        />
...
function auditHeaderEntity(
  audit: AuditDetail,
  vesselOptions: AuditVesselOption[]
): string {
  if (audit.auditee_type === 'OFFICE_DEPT') {
    return audit.auditee_office_dept || 'Office department';
  }

  const vesselId = audit.inspection.vessel_id;
  const normalizedVesselId = normalizeIdentifier(vesselId);
  const vessel = vesselOptions.find(
    (option) => normalizeIdentifier(option.id) === normalizedVesselId
  );

  return vessel ? formatAuditVesselOption(vessel) : vesselId;
}
...
function formatAuditVesselOption(vessel: AuditVesselOption): string {
...
function SubmitGateFailureBanner({ gates }: { gates: SubmitGateErrors }) {
  return (
    <div
      className="border-error-200 rounded-md border bg-error-50 p-4 text-sm text-error-700"
      role="alert"
    >
...
function gateErrorsFromApiError(error: unknown): SubmitGateErrors | null {
  const data = (error as { response?: { data?: { gates?: unknown } } })
    ?.response?.data;
  if (
    !data ||
    !data.gates ||
    typeof data.gates !== 'object' ||
      ...
      >
        {verifyOptions.map((option) => (
          <option key={option || 'blank'} value={option}>
            {option ? formatEnumLabel(option) : 'Not set'}
          </option>
        ))}
      </select>
    </div>
  ...
  const handleStatusChange = (event: ChangeEvent<HTMLSelectElement>) => {
    onChange({
      ...
      status: event.target.value
        ? (event.target.value as AuditScorecardStatus)
        : null,
    });
  };
  ...
  return (
    <div className="rounded-md border border-neutral-200 bg-neutral-50 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-medium text-neutral-800">
            {row.sequence_no}. {row.display_name}
          </p>
          {row.is_vessel_only && (
            <p className="text-xs text-neutral-500">Vessel-only area</p>
          )}
        </div>
        <span
          className={cn(
        ...
        >
          {formatEnumLabel(row.status) || 'Blank'}
        </span>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-[220px_1fr]">
        ...
        <Input
          ...
          value={row.remarks}
          onChange={(event) =>
            onChange({ ...row, remarks: event.target.value })
          }
          placeholder="Remarks"
        />
      </div>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      <div
        ...
        data-eid="MOCKUP-AUDIT-02:detail.nc_count"
      >
      ...
      <div
        className="rounded-md border border-neutral-200 bg-neutral-50 p-3"
        data-eid="MOCKUP-AUDIT-02:detail.obs_count"
      >
        <p className="text-xs font-medium uppercase text-neutral-500">
          Observations Raised
        </p>
        <p className="mt-1 text-2xl font-semibold text-neutral-900">
          {audit.counts.observations}
        </p>
      </div>
      <div className="rounded-md border border-neutral-200 bg-neutral-50 p-3">
        <p className="text-xs font-medium uppercase text-neutral-500">
