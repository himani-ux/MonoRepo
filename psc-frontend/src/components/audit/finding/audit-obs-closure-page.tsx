...
function suggestedObsStep(partB: AuditObsPartB): number {
  if (
    !partB.responded_by_name ||
    !partB.responded_by_rank ||
    !partB.target_closure_date
  )
    return 0;
  if (
...
export function AuditObsClosurePage({
  findingId,
  mode = 'dense',
}: AuditObsClosurePageProps) {
  ...
  useEffect(() => {
    ...
    setPartC({
      ...closure.part_c,
      acceptance_review_date: toInputDate(
        closure.part_c.acceptance_review_date
      ),
      acceptance_signer_at: toInputDateTime(
        closure.part_c.acceptance_signer_at
      ),
    });
    setPartD({
      ...closure.part_d,
      resubmit_by_date: toInputDate(closure.part_d.resubmit_by_date),
      auditor_verification_sign_at: toInputDateTime(
        closure.part_d.auditor_verification_sign_at
      ),
    });
  ...
  return (
    <RootLayout>
      ...
      <div className="space-y-4 p-4">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h1
                  ...
                  data-eid="MOCKUP-OBS-06:obs_partA.ref_no"
                >
                  {closure.part_a.observation_reference_no}
                </h1>
                <Badge variant="secondary">
                  {formatEnumLabel(closure.part_a.observation_category) || 'Observation'}
                </Badge>
                <Badge
                  variant={
                    closure.state === 'MASTER_CLOSED' ? 'success' : 'outline'
                  }
                >
                  {getStatusLabel(closure.state)}
                </Badge>
              </div>
              <p className="mt-1 text-sm text-neutral-500">
                {closure.part_a.description}
              </p>
            </div>
            <HeaderDatum
              label="Vessel / Port"
              value={`${closure.part_a.vessel_id} / ${closure.part_a.port_place}`}
            />
            <HeaderDatum
              label="Target"
              value={closure.part_a.required_closure_deadline || '-'}
            />
            <HeaderDatum label="CAR" value={getStatusLabel(closure.car.status)} />
          </CardContent>
        </Card>
        ...
        {isWizardMode ? (
          ...
          >
            <main className="space-y-4">
                ...
                <CardContent className="space-y-3 p-4">
                  ...
                  <div>
                    ...
                    <h2 className="mt-1 text-xl font-semibold text-neutral-900">
                      {obsWizardSteps[stepIndex]}
                    </h2>
                  </div>
                  <div
                    className="h-2 rounded-full bg-neutral-100"
                    aria-label="Observation wizard progress"
                  >
                    <div
              ...
              {wizardError && (
                <div
                  className="border-error-200 rounded-md border bg-error-50 p-3 text-sm text-error-700"
                  role="alert"
                >
                  {wizardError}
                </div>
              ...
              <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
                <Button
                  ...
                  variant="outline"
                  onClick={goBack}
                  disabled={stepIndex === 0 || updatePart.isPending}
                >
                  <ArrowLeft className="mr-2 h-4 w-4" />
                  Back
                </Button>
                <div className="flex flex-col gap-2 sm:flex-row">
                  <Button
                    ...
                    disabled={
                      updatePart.isPending || closure.state === 'MASTER_CLOSED'
                    }
                  >
                  ...
                  >
                    {stepIndex === obsWizardSteps.length - 1 ? (
            ...
            <aside className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-3 text-sm">
                  <ContextRow
                    label="Reference"
                    value={closure.part_a.observation_reference_no}
                  />
                  <ContextRow
                    label="Deadline"
                    value={closure.part_a.required_closure_deadline || '-'}
                  ...
                  <ContextRow
                    label="Clause"
                    value={
                      closure.part_a.clause_ref_text ||
                      closure.part_a.rule_book_type ||
                      '-'
                    }
                  />
                  <ContextRow
                    label="Evidence"
                    value={closure.part_a.objective_evidence || '-'}
                  />
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent className="space-y-3 text-sm">
                  <ContextRow
                    label="Responder"
                    value={
                      [partB.responded_by_name, partB.responded_by_rank]
                        .filter(Boolean)
                        .join(' / ') || '-'
                    }
                  />
        ...
        {!isWizardMode ? (
          <div className="space-y-4" data-testid="obs-dense-layout">
            <Card>
              <CardHeader>
              ...
              <CardContent className="grid gap-3 lg:grid-cols-2">
                <ReadOnlyDatum
                  label="Auditor"
                  value={`${closure.part_a.auditor_name} / ${closure.part_a.auditor_organisation}`}
                  eid="MOCKUP-OBS-06:obs_partA.auditor"
                />
                <ReadOnlyDatum
                  label="Clause"
                  value={
                  ...
                  eid="MOCKUP-OBS-06:obs_partA.ref"
                />
                <ReadOnlyDatum
                ...
                <ReadOnlyDatum
                  label="Issued Date"
                  value={closure.part_a.observation_issued_date || '-'}
                />
              </CardContent>
            </Card>
            ...
            <SectionCard title="Part B - Master / HOD Response">
              <FieldInput
                ...
                onChange={(value) =>
                  setPartB({ ...partB, responded_by_name: value })
                }
                eid="MOCKUP-OBS-06:obs_wizard_step1.responder"
              />
              <FieldInput
                id="responded_by_rank"
                label="Responded By Rank"
                value={partB.responded_by_rank}
                onChange={(value) =>
                  setPartB({ ...partB, responded_by_rank: value })
                }
              />
              <FieldInput
                ...
                value={partB.target_closure_date || ''}
                onChange={(value) =>
                  setPartB({ ...partB, target_closure_date: value })
                }
                eid="MOCKUP-OBS-06:obs_wizard_step1.target_date"
              />
              <FieldInput
                ...
                onChange={(value) =>
                  setPartB({ ...partB, actual_closure_date: value })
                }
              />
              <FieldTextarea
                ...
                value={partB.immediate_action_text}
                onChange={(value) =>
                  setPartB({ ...partB, immediate_action_text: value })
                }
                eid="MOCKUP-OBS-06:obs_wizard_step2.fields"
              />
              <FieldTextarea
                id="root_cause_text"
                label="Root Cause"
                value={partB.root_cause_text}
                onChange={(value) =>
                  setPartB({ ...partB, root_cause_text: value })
                }
              />
              <FieldTextarea
                id="corrective_action_text"
                label="Corrective Action"
                value={partB.corrective_action_text}
                onChange={(value) =>
                  setPartB({ ...partB, corrective_action_text: value })
                }
              />
              <FieldTextarea
                id="preventive_action_text"
                label="Preventive Action"
                value={partB.preventive_action_text}
                onChange={(value) =>
                  setPartB({ ...partB, preventive_action_text: value })
                }
              />
              <label className="flex items-center gap-2 text-sm text-neutral-700">
                <input
                  ...
                  onChange={(event) =>
                    setPartB({
                      ...partB,
                      sms_amendment_required: event.target.checked,
                    })
                  }
              ...
              <FieldInput
                id="sms_amendment_doc_ref"
                label="SMS Amendment Reference"
                value={partB.sms_amendment_doc_ref}
                onChange={(value) =>
                  setPartB({ ...partB, sms_amendment_doc_ref: value })
                }
              />
              <FieldInput
                id="master_sign_name"
                ...
                value={partB.master_sign_name}
                onChange={(value) =>
                  setPartB({ ...partB, master_sign_name: value })
                }
                eid="MOCKUP-OBS-06:obs_partB.master_sign"
              />
              <FieldInput
                id="master_sign_at"
                label="Master Signature Time"
                type="datetime-local"
                value={partB.master_sign_at || ''}
                onChange={(value) =>
                  setPartB({ ...partB, master_sign_at: value })
                }
              />
              <SaveRow
                ...
                onSave={() =>
                  savePart('part-b', {
                    ...partB,
                    target_closure_date: fromInputDate(
                      partB.target_closure_date || ''
                    ),
                    actual_closure_date: fromInputDate(
                      partB.actual_closure_date || ''
                    ),
                    master_sign_at: fromInputDateTime(
                      partB.master_sign_at || ''
                    ),
                  })
                }
              />
            ...
            <SectionCard title="Part C - DPA Review">
              <ReadOnlyNote>
                Part C records audit-trail review only. It does not gate or
                reopen the Observation state.
              </ReadOnlyNote>
              <FieldInput
                id="acceptance_review_date"
                label="Review Date"
                type="date"
                value={partC.acceptance_review_date || ''}
                onChange={(value) =>
                  setPartC({ ...partC, acceptance_review_date: value })
                }
                eid="MOCKUP-OBS-06:obs_partC.dpa_review"
              />
              <SelectField
                id="acceptance_decision"
                label="Decision"
                value={partC.acceptance_decision}
                options={OBS_ACCEPTANCE_DECISIONS}
                onChange={(value) =>
                  setPartC({ ...partC, acceptance_decision: value })
                }
              />
              <FieldTextarea
                id="acceptance_adequacy_text"
                label="Adequacy Review"
                value={partC.acceptance_adequacy_text}
                onChange={(value) =>
                  setPartC({ ...partC, acceptance_adequacy_text: value })
                }
              />
              <FieldTextarea
                id="acceptance_return_reason"
                label="Return Reason"
                value={partC.acceptance_return_reason}
                onChange={(value) =>
                  setPartC({ ...partC, acceptance_return_reason: value })
                }
              />
              <FieldInput
                id="acceptance_signer_name"
                label="Reviewed By"
                value={partC.acceptance_signer_name}
                onChange={(value) =>
                  setPartC({ ...partC, acceptance_signer_name: value })
                }
              />
              <FieldInput
                id="acceptance_signer_at"
                label="Review Time"
                type="datetime-local"
                value={partC.acceptance_signer_at || ''}
                onChange={(value) =>
                  setPartC({ ...partC, acceptance_signer_at: value })
                }
              />
              <SaveRow
                pending={updatePart.isPending}
                onSave={() =>
                  savePart('part-c', {
                    ...partC,
                    acceptance_review_date: fromInputDate(
                      partC.acceptance_review_date || ''
                    ),
                    acceptance_signer_at: fromInputDateTime(
                      partC.acceptance_signer_at || ''
                    ),
                  })
                }
              />
            ...
            <SectionCard title="Part D - Auditor Verification">
              <ReadOnlyNote>
                Part D is recorded after Master closure and does not change the
                terminal state.
              </ReadOnlyNote>
              <FieldInput
                id="verifying_auditor_name"
                label="Verifying Auditor"
                value={partD.verifying_auditor_name}
                onChange={(value) =>
                  setPartD({ ...partD, verifying_auditor_name: value })
                }
                eid="MOCKUP-OBS-06:obs_partD.verification"
              />
              <FieldInput
                id="verifying_authority_org"
                label="Authority / Org"
                value={partD.verifying_authority_org}
                onChange={(value) =>
                  setPartD({ ...partD, verifying_authority_org: value })
                }
              />
              <SelectField
                id="verification_method"
                label="Verification Method"
                value={partD.verification_method}
                options={OBS_VERIFICATION_METHODS}
                onChange={(value) =>
                  setPartD({ ...partD, verification_method: value })
                }
              />
              <FieldTextarea
                id="auditor_remarks_text"
                label="Auditor Remarks"
                value={partD.auditor_remarks_text}
                onChange={(value) =>
                  setPartD({ ...partD, auditor_remarks_text: value })
                }
              />
              <SelectField
                id="closure_status"
                label="Closure Status"
                value={partD.closure_status}
                options={OBS_CLOSURE_STATUSES}
                onChange={(value) =>
                  setPartD({ ...partD, closure_status: value })
                }
              />
              <FieldInput
                id="resubmit_by_date"
                label="Resubmit By"
                type="date"
                value={partD.resubmit_by_date || ''}
                onChange={(value) =>
                  setPartD({ ...partD, resubmit_by_date: value })
                }
              />
              <FieldInput
                id="auditor_verification_sign_at"
                label="Verification Time"
                type="datetime-local"
                value={partD.auditor_verification_sign_at || ''}
                onChange={(value) =>
                  setPartD({ ...partD, auditor_verification_sign_at: value })
                }
              />
              <SaveRow
                pending={updatePart.isPending}
                onSave={() =>
                  savePart('part-d', {
                    ...partD,
                    resubmit_by_date: fromInputDate(
                      partD.resubmit_by_date || ''
                    ),
                    auditor_verification_sign_at: fromInputDateTime(
                      partD.auditor_verification_sign_at || ''
                    ),
                  })
                }
              />
...
function ObsWizardStep({
    ...
    return (
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <div
            className="space-y-2"
            data-eid="MOCKUP-OBS-06:obs_wizard_step1.responder"
          >
            ...
            <Input
              ...
              onChange={(event) =>
                onPartBChange({
                  ...partB,
                  responded_by_name: event.target.value,
                })
              }
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(event) =>
                onPartBChange({
                  ...partB,
                  responded_by_rank: event.target.value,
                })
              }
            />
          </div>
          <div
            ...
            data-eid="MOCKUP-OBS-06:obs_wizard_step1.target_date"
          >
            <Label htmlFor="obs_wizard_target_closure_date">
              Target Closure Date
            </Label>
            <Input
              ...
              onChange={(event) =>
                onPartBChange({
                  ...partB,
                  target_closure_date: event.target.value,
                })
              }
            />
          </div>
    ...
    return (
      ...
      >
        <div
          ...
          data-eid="MOCKUP-OBS-06:obs_wizard_step2.fields"
        >
          <WizardTextarea
            label="Immediate action"
            value={partB.immediate_action_text}
            disabled={terminal}
            onChange={(value) =>
              onPartBChange({ ...partB, immediate_action_text: value })
            }
          />
          <WizardTextarea
            label="Root cause"
            ...
            disabled={terminal}
            onChange={(value) =>
              onPartBChange({ ...partB, root_cause_text: value })
            }
          />
          <WizardTextarea
            label="Corrective action"
            ...
            disabled={terminal}
            onChange={(value) =>
              onPartBChange({ ...partB, corrective_action_text: value })
            }
          />
          <WizardTextarea
            label="Preventive action"
            ...
            disabled={terminal}
            onChange={(value) =>
              onPartBChange({ ...partB, preventive_action_text: value })
            }
          />
          <label className="flex items-center gap-2 text-sm text-neutral-700">
            <input
          ...
          {partB.sms_amendment_required && (
            <Input
              ...
              onChange={(event) =>
                onPartBChange({
                  ...partB,
                  sms_amendment_doc_ref: event.target.value,
                })
              }
            />
          )}
  ...
  return (
    <PromptCard
      ...
      hint="This signature makes the Observation Master Closed. Parts C and D stay audit-trail only."
    >
      <div
        className="grid gap-3 sm:grid-cols-2"
        data-eid="MOCKUP-OBS-06:obs_partB.master_sign"
      >
        <div className="space-y-2">
          <Label htmlFor="obs_wizard_actual_closure_date">
            Closure Date for Signature
          </Label>
          <Input
            ...
            onChange={(event) =>
              onPartBChange({
                ...partB,
                actual_closure_date: event.target.value,
              })
            }
          />
        </div>
        <div className="space-y-2">
          <Label htmlFor="obs_wizard_master_sign_name">
            Master Signer for Closure
          </Label>
          <Input
            ...
            disabled={terminal}
            onChange={(event) =>
              onPartBChange({ ...partB, master_sign_name: event.target.value })
            }
          />
        </div>
        <div className="space-y-2 sm:col-span-2">
          ...
          <Input
            ...
            disabled={terminal}
            onChange={(event) =>
              onPartBChange({ ...partB, master_sign_at: event.target.value })
            }
          />
        </div>
      </div>
  ...
  return (
    <div className="space-y-2">
      ...
      <p
        className={cn(
          'text-xs',
          value.length >= 20 ? 'text-success-700' : 'text-neutral-500'
        )}
      >
