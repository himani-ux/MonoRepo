...
This report does not mark any Audit journey as passed. The browser rerun was not executed because the local `journey/surface-check` runner package is absent in this checkout and the required journey runtime environment variables were not present in the shell: `JOURNEY_USERNAME`, `JOURNEY_PASSWORD`, `JOURNEY_AUDIT_ID`, `JOURNEY_NC_FINDING_ID`, `JOURNEY_OBS_FINDING_ID`, `JOURNEY_EXTERNAL_AUDIT_ID`, `JOURNEY_ACTING_HOD_ROUTE`, and `APP_BASE_URL`.
...
| --- | --- | --- | --- |
| JOURNEY-1 | BLOCKED | `/audit/plans`, plan `A1170000-0000-0000-0000-000000000001` | Browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-001.spec.ts:5`: `Audit Plans`, `Register Audit`, `Create routine plan entry`, `Register rows`, `OPM F 713`. |
| JOURNEY-2 | BLOCKED | `/inspections/new` | Browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-002.spec.ts:5`: `Audit Classification`, `Audit Subtype`, `Lead Auditor`, `Register Audit`. |
...
| JOURNEY-4 | BLOCKED | `/audit/findings/A1170000-0000-0000-0000-000000000007/nc/wizard` | NC finding exists, but browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-004.spec.ts:6`: `Root Cause`, `Save and Continue`, `RCA`, `wizard`. |
...
| JOURNEY-9 | BLOCKED | `/audit/plans`, plan `A1170000-0000-0000-0000-000000000001` | Browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-009.spec.ts:5`: `OPM F 713`, `Request extension`, `Cancel`, `Extension`. |
| JOURNEY-10 | BLOCKED | `/audit/plans`, plan `A1170000-0000-0000-0000-000000000001` | Browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-010.spec.ts:5`: `Create additional audit`, `Additional reason`, `Trigger type`, `Additional`. |
| JOURNEY-11 | BLOCKED | `/dpa/notifications/failed` and `/dpa/scan-validation-queue` | Browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-011.spec.ts:6` and `:9`: `Retry`, `notified offline`, `Accept`, `rescan`, queue empty states. |
| JOURNEY-12 | BLOCKED | `/audit/external/new`; optional `/audit/external/<external_audit_id>` | Registration route has source evidence at `psc-frontend/src/App.tsx:278`, but browser rerun is blocked by missing account/env and runner package. Optional close-out route is also blocked by missing local external audit detail record; read-only SQL found no external audit row. Expected controls from `tests/journeys/journey-012.spec.ts:6` and `:12`: `External Audit Definition`, `External Audit Org UUID`, `External Lead Auditor`, `Register External Audit`, `External Audit Close-out`, `certificate impact`, `Confirm External Closure`. |
| JOURNEY-13 | BLOCKED | `/inspections/new` | Browser rerun blocked by missing account/env and missing runner package. Expected controls from `tests/journeys/journey-013.spec.ts:5`: `Office Department`, `OFFICE_DEPT`, `Department`, `Audit Scope`. |
...
- Restore or provide the journey runner package expected by `tests/journeys/README.md`: `journey/surface-check`.
