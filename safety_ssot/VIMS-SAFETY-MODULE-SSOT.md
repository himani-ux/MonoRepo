...
| Phase 6 Office Review | Office reviews the incident and either approves it or sends it back | Office-side users see Office Comments/lesson learnt, typed-name approval, and a send-for-rework Comment box. Ship-side users see only Office Comments/lesson learnt when a comment exists. Root/action counters, pre-approval summary cards, approval-role wording, and send-back target selection are not shown. | Saves Office Comments on `vims_safety_incident.office_comment`, runs checks, captures signs, supports PDF preview/download for office-side users, lets PIC or DPA accept any risk band, or sends back to action rework with a fixed backend target phase |
