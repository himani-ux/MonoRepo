...
| Phase 3 Corrective Action | Record what must be corrected for this incident | Description and Due date. The owner/checker card is not shown; backend compatibility values are supplied by the frontend where required. | Saves the corrective action; shows a success message and scrolls to the saved action; saved corrective cards expose Edit and update the existing recommendation/corrective-action row instead of creating duplicates; continues to Preventive Action |
| Phase 4 Preventive Action | Record what will prevent recurrence | Preventive action description, Due date, and one shared How much will this reduce risk? answer for the screen. Remaining risk, risk-confirmation checkbox, theme, effort, and "Prevent It Happening Again" wording are not shown in the current UI. | Saves the preventive action and linked due-date action using the shared risk-reduction answer; shows a success message and scrolls to the saved action; saved preventive cards expose Edit and update the existing recommendation/action rows instead of creating duplicates; continues directly to Office Review |
| Phase 5 Add Evidence | Collect and describe proof before it disappears | One Documents evidence section with Attachment, Title, and Description. The user repeats the form for as many attachments as needed. Witness Statement remains available with vessel crew/Other witness selection, Remark, and Upload witness statement. Evidence Check is not shown in the current Evidence UI. | Saves new attachments under the legacy PAPER evidence tab; shows a success message after document/witness save and scrolls to saved content; saved document cards expose Edit for title/description metadata without replacing the file, and saved Witness Statement cards expose Edit to update the existing witness row; keeps legacy evidence rows for older records, and can still load simplified witness statements, interviews, and deadline-task updates |
| Phase 6 Office Review | Office reviews the incident and either approves it, sends it back, or issues a targeted fleet alert | Office-side users see Office Comments/lesson learnt, typed-name approval, a Fleet Alert action with multi-ship selection, and a send-for-rework Comment box. Ship-side users see Office Comments/lesson learnt; when no note exists, the screen shows "Office comment is not added yet." Root/action counters, pre-approval summary cards, approval-role wording, send-back target selection, and Phase 7 PDF availability warning text are not shown. | Saves Office Comments on `vims_safety_incident.office_comment`, runs checks, captures signs, supports PDF preview/download for office-side users without waiting for Phase 7 acceptance, lets PIC or DPA accept any risk band, can send in-app plus email Fleet Alert notifications only to selected `VesselData` ships, or sends back to action rework with a fixed backend target phase |
| Phase 7 Loss Evaluation | Evaluate consequence, likelihood, operational loss, repair/injury details, and estimated costs before closure | First choose Loss Evaluation type: Incident Report or Injury Report; then Risk Assessment; Other Details; Cost Evaluation; Estimated Costs; closing note | Authorized ship-side and office-side users with incident form access and vessel scope can open and save one editable Loss Evaluation record per incident in `vims_safety_incident_loss_evaluation` without waiting for Office Review approval; the saved `report_type` controls whether Incident Report or Injury Report fields are shown on reload and in PDF cost blocks; existing rows without `report_type` keep the old injury-record fallback; closure is allowed after Loss Evaluation is saved and remains an office close action |
...
|------|---------------------------|-------|
| Incident identity | `incident_number`, `draft_reference`, `vessel_id`, `vessel_code`, `state`, `current_phase`, `schema_version` | Draft and formal numbering coexist; formal numbering is used after submit/office communication. Internal UUID/auto-save status chips are not shown in the current Phase 1 or Phase 4 user headers. |
| Phase 1 timing and story | `occurred_at`, `reported_at`, `narrative` | Narrative is required with meaningful detail. The former first-check field is not exposed by the current UI/API/PDF flow and remains only as a legacy database column. |
| Phase 1 classification | `incident_type_id`, `loss_type_primary_id`, `loss_type_secondary_id`, `loss_type_tertiary_id`, `loss_type_other`, `risk_band` | Up to three standard loss types plus Other text where selected; duplicates are blocked. |
...
| Phase 1 reporting context | `shore_assistance_required`, `vessel_location`, `onboard_location`, `departure_date`, `vessel_condition` | These nullable incident-level fields are visible in the main Incident Report section and are shared by incident and injury reporting. Shore Assistance Required is placed beside Report time; Location of Vessel, Location on Board, Departure Date, and Vessel Condition remain in the reporting-context fields below. Last Port remains a legacy database/API/PDF-fallback field but is not shown or sent by the current Phase 1 frontend. |
...
| Injury handling | `external_party_injury` object now functions as the Phase 1 injury record with `injured_person_type` = `CREW` or `NON_CREW` | Non-crew keeps person/company/party type/injury level/notes. Crew captures rank from active vessel crew ranks, age, and Type of Activity. Crew injury also captures investigation narrative and OCIMF flags. Estimated costs are optional; the UI asks whether to add them, shows the cost fields only on Yes, and does not block continuation on No. Nature/source/body-area dropdowns and Type of Activity are loaded from `vims_safety_injury_dropdown_option` using field keys `NATURE_OF_INJURY`, `SOURCE_OF_INJURY`, `AFFECTED_BODY_AREA`, and `TYPE_OF_ACTIVITY`. |
...
| Phase 4 supporting tools | Witness Statement and optional Checklist | Supporting tools are collapsed by default and opened only when needed. Witness Statement opens the witness page directly and shows vessel crew/Other witness selection, Remark, and Upload witness statement. Checklist task cards show the item, status, and action controls without due-date wording or due-date values. Evidence Check / Evidence Matrix is legacy compatibility only and is not shown as a current Phase 4 tool. |
...
| Action phases | recommendation tier/theme/title/description/rationale storage; corrective action verifier/due date/status; ALARP likelihood reduction, residual risk, attestation; purchase requisition link | Current UI exposes only Corrective Action and Preventive Action. The legacy `LESSONS_LEARNT` tier remains readable for old records/API compatibility but is not a current screen, tab, dropdown option, or PDF-selector default. Current UI does not expose a separate title field; title is derived from the entered description. Corrective Action shows Description and Due date only; the owner/checker card is removed. Preventive Action shows Description, Due date, and one shared How much will this reduce risk? answer for the screen; the frontend sends that shared value as the compatibility `estimated_likelihood_reduction` value for preventive saves and saved preventive cards do not repeat risk reduction per row. Current UI does not expose or send recommendation rationale / "Why is this needed?", theme, effort, Remaining risk, or risk-confirmation checkbox; incident PDF does not print stored action rationale. |
| Phase 6 Office Review | approvals, HOD sign where required, PIC/DPA Office Review decision, send-for-rework comment, `office_comment`, Fleet Alert selected vessel IDs | PIC or DPA can accept, close, send to rework, or issue an Incident Fleet Alert for every risk band. Office Comments/lesson learnt is free text with no word/character limit and remains separate from the later closure reason. Fleet Alert requires at least one selected active `VesselData` ship and sends in-app plus email alerts only to those selected ships, using `VesselData.email` for email delivery. Ship-side users always see the Office Comments/lesson learnt card; if no note exists, it shows "Office comment is not added yet." |
| Phase 7 Loss Evaluation | `report_type`, consequence, likelihood, risk level, master/chief engineer names, incident repair/loss/cost fields, injury safe-working-practice/rest/repatriation/hospitalization/evacuation/cost fields, closing note | Stored in `vims_safety_incident_loss_evaluation`. The current UI requires the user to choose Incident Report or Injury Report first; the selected nullable `report_type` is persisted by migration `0056_incident_loss_evaluation_report_type` and controls visible fields on reload and Loss Evaluation PDF cost blocks. Existing rows without `report_type` keep the old fallback that infers Injury Report when an injury row exists. Authorized ship-side and office-side users with `SAF_F_001` access and vessel scope can open and save Loss Evaluation before Office Review approval; incident closure still requires the Loss Evaluation record to be saved and the office close action to supply a closure note. Safe Working Practice uses the existing `vims_safety_injury_dropdown_option` master with field key `SAFE_WORKING_PRACTICE`; CR-048 seeds the active Code of Safe Working Practices options and deactivates stale placeholder choices. |
...
- The incident register no longer shows the implementation-facing Current Scope card. Routine incident pages also avoid duplicate module breadcrumbs/page headings and internal incident UUID/status chips. The phase tabs are the single phase number/name indicator; phase content does not repeat separate Phase X/phase-title header cards.
...
- Before preview/download, the current user-facing selector defaults these sections to selected: Summary, Reporter Details, Injury Details, Estimated Cost, Root Cause, Evidence (Documents), Corrective / Preventive Actions, and Signature. The legacy backend `lessons_learned` section key remains supported only for old/direct export compatibility.
- The PDF title prints `Injury Report` when a Phase 1 injury row exists; otherwise it prints `Incident Report`.
- Summary prints the incident classification and context rows. Odd trailing summary rows leave the unused cells blank; filler cells do not print `Not recorded`.
- Office Review comments and Closure reason are excluded from Summary and print in the final Closure/Office Review area immediately before Signature when present.
- Evidence (Documents) prints each saved attachment as a separate document block using the saved title when available, with Description and File rows. It does not print generic numbered labels such as `Attachment 1` / `Attachment 2`, and it does not print legacy evidence-note-only rows. Saved Witness Statements print with Witness name, What the witness said, and Remark.
- Current action PDF blocks print the saved description once inside their detail block and do not print recommendation rationale / "Why is this needed?" text. Legacy Lessons Learned PDF output remains supported only when the legacy section is explicitly requested or reached through older direct export defaults.
- Signature rows remain visible even when unsigned; unsigned rows show as pending.
...
| `/api/safety/incidents/{id}/phase-5/preflight/` | GET | User-facing Phase 6 Office Review/PDF preview info |
| `/api/safety/incidents/{id}/phase-5/hod-signature/` | POST | Capture HOD signature |
| `/api/safety/incidents/{id}/phase-5/accept/` | POST | PIC or DPA accepts any risk band and can save Office Comments |
...
| `/api/safety/incidents/{id}/phase-5/send-back/` | POST | PIC or DPA sends incident back for rework; the current UI sends a fixed action-rework target and does not expose a phase picker |
| `/api/safety/incidents/{id}/fleet-alert/` | GET/POST | Office Review Incident Fleet Alert. GET returns active ships from `VesselData`; POST requires selected vessel IDs and sends in-app plus email only to those selected ships |
| `/api/safety/incidents/{id}/phase-6/` | GET | User-facing Phase 7 Loss Evaluation workspace for authorized ship-side or office-side users with incident form access and vessel scope; returns `choices.report_type` and the saved/effective report type |
...
| `/api/safety/incidents/{id}/phase-6/close/` | POST | Close after Loss Evaluation is saved and closure note is supplied |
...
- Office Review: PIC or DPA with process permission can accept, close, or send to rework for every risk band. FM-specific RED Office Review is legacy-only and is not required in the current implemented flow. Office Comments are saved separately from closure reason.
- Loss Evaluation save/close: authorized ship-side and office-side users with `SAF_F_001` access and vessel scope can save Loss Evaluation before Office Review approval; closure remains an office close action for PIC or DPA after the Loss Evaluation record exists.
...
| D-MAINT-CR047 | **Incident visible Phase 7 is Loss Evaluation:** CR-047 supersedes CR-043 only where visible Phase 7 was named Check Actions. Backend `current_phase` 8 and compatibility route `/safety/incidents/:id/phase-6` remain unchanged, but the workspace now saves `vims_safety_incident_loss_evaluation` and closes only after that record exists. | 2026-07-06 | Maintain-mode structural change CR-047: Phase 7 now captures risk, loss, repair/injury, and cost evaluation for Incident Report and Injury Report records. |
...
| D-MAINT-CR051 | **Incident Fleet Alert from Office Review:** Office-side Phase 6 Office Review shows a Fleet Alert action below Accept / Close. PIC or DPA opens the ship selector, selects one or more active `VesselData` ships, and the system sends in-app `INCIDENT_FLEET_ALERT` notifications plus emails only to the selected ships using `VesselData.email`. No new Safety table or migration is introduced. | 2026-07-07 | Maintain-mode feature CR-051: users needed the Near Miss fleet-alert concept available for Incidents with explicit selected-ship targeting. |
