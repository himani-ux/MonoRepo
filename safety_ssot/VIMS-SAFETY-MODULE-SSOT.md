...
| Phase 4 Preventive Action | Record what will prevent recurrence | Preventive action description, Due date, and How much will this reduce risk? Remaining risk, risk-confirmation checkbox, theme, effort, and "Prevent It Happening Again" wording are not shown in the current UI. | Saves the preventive action and linked due-date action; shows a success message and scrolls to the saved action; saved preventive cards expose Edit and update the existing recommendation/action rows instead of creating duplicates; continues directly to Office Review |
...
| `/api/safety/incidents/{id}/phase-6/` | GET | User-facing Phase 7 Loss Evaluation workspace |
| `/api/safety/incidents/{id}/phase-6/` | PATCH | Save editable Phase 7 Loss Evaluation fields |
...
- Loss Evaluation save/close: PIC or DPA can act for every risk band once Office Review has advanced the record to backend `current_phase` 8.
