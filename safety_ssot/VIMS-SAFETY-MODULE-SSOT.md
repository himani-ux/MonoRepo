...
| Phase 7 Loss Evaluation | Evaluate consequence, likelihood, operational loss, repair/injury details, and estimated costs before closure | Risk Assessment; Other Details; Cost Evaluation; Estimated Costs; closing note | Authorized ship-side and office-side users with incident form access and vessel scope can open and save one editable Loss Evaluation record per incident in `vims_safety_incident_loss_evaluation` without waiting for Office Review approval; Incident Report and Injury Report records show different field groups; closure is allowed after Loss Evaluation is saved and remains an office close action |
...
| Phase 7 Loss Evaluation | consequence, likelihood, risk level, master/chief engineer names, incident repair/loss/cost fields, injury safe-working-practice/rest/repatriation/hospitalization/evacuation/cost fields, closing note | Stored in `vims_safety_incident_loss_evaluation`. Authorized ship-side and office-side users with `SAF_F_001` access and vessel scope can open and save Loss Evaluation before Office Review approval; incident closure still requires the Loss Evaluation record to be saved and the office close action to supply a closure note. Safe Working Practice uses the existing `vims_safety_injury_dropdown_option` master with field key `SAFE_WORKING_PRACTICE`; CR-048 seeds the active Code of Safe Working Practices options and deactivates stale placeholder choices. |
...
| `/api/safety/incidents/{id}/phase-6/` | GET | User-facing Phase 7 Loss Evaluation workspace for authorized ship-side or office-side users with incident form access and vessel scope |
| `/api/safety/incidents/{id}/phase-6/` | PATCH | Save editable Phase 7 Loss Evaluation fields without requiring Office Review approval/backend `current_phase` 8 |
