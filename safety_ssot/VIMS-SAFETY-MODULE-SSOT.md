...
- Near Miss numbering follows D-GAP-C1 with Near Miss-specific timing: vessel-side records keep `DRAFT-{VslCode}/{YYYY}/T{nnn}` while pending vessel review, then receive the shared formal `{VslCode}/{YYYY}/{NNN}` number when the state becomes `READY_FOR_OFFICE_COMMENTS`. Already formal Near Miss numbers are not reallocated.
...
| D-MAINT-CR155 | **Near Miss formal numbering at office transition:** Near Miss uses `vims_safety_incident.incident_number` only. Vessel-side records keep `DRAFT-{VslCode}/{YYYY}/T{nnn}` until `READY_FOR_OFFICE_COMMENTS`; that transition assigns the shared formal `{VslCode}/{YYYY}/{NNN}` number through the existing formal allocator. The final number has no `DRAFT-` prefix or `/T` marker, and already formal numbers are idempotently preserved. | 2026-08-21 | Maintain-mode numbering correction CR-155: office-side Near Miss registers must not continue showing draft references after vessel submit/resubmit. |
...
| D-MAINT-CR135 | **Safety Dashboard PSC-style summary view:** Current `/safety/dashboard/` defaults to compact scope/period/vessel controls, Safety score, KPI cards, and export controls. Repeat issues, top repeat causes, corrective-action age, Heinrich Ratio, and SOI Compliance % stay hidden until the user opens **Show details**. Existing calculations, endpoints, permissions, and auditor export behavior remain unchanged. Supersedes D-MAINT-CR084 only for the current Safety Dashboard default presentation and details-button wording. | 2026-08-13 | Maintain-mode dashboard simplification CR-135: users found the Safety Dashboard still chaotic and asked for a cleaner PSC-dashboard-style layout. |
...
| D-MAINT-CR136 | **Safety Dashboard charted summary:** `/safety/dashboard/` keeps the CR-135 simplified PSC-style layout but adds compact open-work and score-breakdown chart panels to the default summary area. These charts use the existing composite dashboard response and do not change dashboard calculations, endpoints, permissions, exports, or the **Show details** analytics behavior. Supersedes D-MAINT-CR135 only where the default summary had no charted visual panels. | 2026-08-13 | Maintain-mode UI enhancement CR-136: user found the simplified dashboard too plain and requested styling plus charts without bringing back dashboard clutter. |
| D-MAINT-CR137 | **Safety Dashboard graph/chart summary:** `/safety/dashboard/` keeps the simplified CR-135 flow and CR-136 summary chart area, but the first-screen visuals render as an open-work donut chart and a score-breakdown horizontal chart instead of simple bar indicators. The charts use the existing composite dashboard response and do not change calculations, endpoints, permissions, exports, or the **Show details** analytics behavior. Supersedes D-MAINT-CR136 only where the summary visuals were simple progress-style bars. | 2026-08-13 | Maintain-mode UI enhancement CR-137: user clarified they wanted graphs and charts. |
| D-MAINT-CR138 | **Safety Dashboard full pie chart:** `/safety/dashboard/` keeps the open-work pie chart but renders it as a full separated-slice pie instead of a donut/ring chart. The chart uses the existing composite dashboard response and does not change calculations, endpoints, permissions, exports, or the **Show details** analytics behavior. Supersedes D-MAINT-CR137 only for the open-work pie chart style. | 2026-08-13 | Maintain-mode UI tweak CR-138: user wanted the pie chart to remain, but in a different style. |
| D-MAINT-CR139 | **Safety Dashboard Reporting trend removal:** `/safety/dashboard/` no longer renders the Reporting trend / Heinrich Ratio card and no longer calls the dashboard Heinrich query from this screen. Repeat issues, top repeat causes, corrective-action age, and SOI Compliance % remain available behind **Show details**. The backend Heinrich/reporting-trend endpoint and logic remain retained for compatibility. Supersedes D-MAINT-CR135 through D-MAINT-CR138 only where they said Heinrich Ratio / Reporting trend remained available on the current dashboard. | 2026-08-14 | Maintain-mode UI removal CR-139: user requested the Reporting trend card be removed from the Safety Dashboard. |
| D-MAINT-CR140 | **Safety Dashboard score breakdown explanation:** `/safety/dashboard/` uses the existing composite `component_scores` payload but shows Score breakdown as a ranked score-health list instead of a raw horizontal chart. The lowest component appears first, the weakest component is highlighted as pulling the score down most, and every component is labelled Good, Watch, or Needs attention. Backend calculation, endpoint, permissions, exports, and **Show details** analytics behavior remain unchanged. Supersedes D-MAINT-CR136 through D-MAINT-CR139 only where they described Score breakdown as a horizontal chart. | 2026-08-14 | Maintain-mode UI logic refinement CR-140: user found the Score breakdown card confusing and asked to make its logic easier to understand. |
| D-MAINT-CR141 | **Safety Dashboard overall status and total KPI cards:** `/safety/dashboard/` shows Overall status instead of the primary numeric Safety score. Top KPI cards show total incidents, total near misses, total findings, total corrective actions, and SOI Compliance % for the selected period/scope. The Open work pie chart remains the open/overdue-count view. Dashboard response now includes additive total metric fields while preserving existing open-count fields and score calculation for compatibility. Supersedes D-MAINT-CR135 through D-MAINT-CR140 only where they described the first summary card as a numeric Safety score or top KPI cards as open-item cards. | 2026-08-14 | Maintain-mode dashboard refinement CR-141: user wanted a clearer first card and total counts in the boxes because open counts already appear in the pie chart. |
| D-MAINT-CR142 | **Safety Dashboard hover help:** `/safety/dashboard/` main summary cards expose short plain-language hover help so new users can understand each card without permanent extra text. Hover help explains card meaning only and does not change dashboard metrics, calculations, endpoints, permissions, exports, or **Show details** behavior. | 2026-08-14 | Maintain-mode dashboard usability refinement CR-142: user asked for simplified detail info on hover so new users are not confused. |
