...
| Phase 6 Office Review | Office reviews the incident and either approves it or sends it back | Office-side users see Office Comments/lesson learnt, typed-name approval, and a send-for-rework Comment box. Ship-side users see Office Comments/lesson learnt; when no note exists, the screen shows "Office comment is not added yet." Root/action counters, pre-approval summary cards, approval-role wording, send-back target selection, and Phase 7 PDF availability warning text are not shown. | Saves Office Comments on `vims_safety_incident.office_comment`, runs checks, captures signs, supports PDF preview/download for office-side users without waiting for Phase 7 acceptance, lets PIC or DPA accept any risk band, or sends back to action rework with a fixed backend target phase |
...
| Phase 6 Office Review | approvals, HOD sign where required, PIC/DPA Office Review decision, send-for-rework comment, `office_comment` | PIC or DPA can accept, close, or send to rework for every risk band. Office Comments/lesson learnt is free text with no word/character limit and remains separate from the later closure reason. Ship-side users always see the Office Comments/lesson learnt card; if no note exists, it shows "Office comment is not added yet." |
