# Candidate — golden expected fan-out generator output for bundle feat-001
#
# Frozen candidate-format contract (review C3): one `## JOURNEY-CANDIDATE`
# block per journey (NEVER a final JOURNEY-ID — the merge assigns ids),
# followed by exactly one `json field_sources` fence. This file is also the
# worked example embedded in fanout-generator.md — keep them in sync.
