...
class TrackedItemUploadPdfView(generics.GenericAPIView):
    ...
    def post(self, request, tracked_item_id: str, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            existing_blob = pdf_repository.get_blob_for_tracked_item_sha(
                tracked_item_id=str(tracked_item_id),
...
class TrackedItemRemovePdfView(generics.GenericAPIView):
    permission_classes = [IsAuthenticated, HasTrackedItemReadPermission]

    def post(self, request, tracked_item_id: str, *args, **kwargs):
        if not has_request_certs_perm(request, TRACKED_ITEM_FORM_ID, TRACKED_ITEM_WRITE_PROCESS_ID):
            return Response({"detail": "You do not have access to remove Certs PDFs."}, status=status.HTTP_403_FORBIDDEN)
        ...
        active_blob_id = str(current.get("pdf_attachment_id") or "").strip()
        if not active_blob_id:
            return Response({"detail": "No active PDF is attached to this certificate."}, status=status.HTTP_400_BAD_REQUEST)

        active_blob = pdf_repository.get_blob(active_blob_id)
        if not active_blob:
            return Response({"detail": "Active PDF record was not found."}, status=status.HTTP_404_NOT_FOUND)

        reason = str(request.data.get("reason") or "").strip()
        if not reason:
            return Response({"reason": "Enter a reason for removing this PDF."}, status=status.HTTP_400_BAD_REQUEST)

        actor_id = resolve_actor_id(request.user)
        previous_versions = [
            blob
            for blob in repository.list_pdf_versions(str(tracked_item_id))
            if str(blob.get("blob_id")) != active_blob_id and not bool(blob.get("is_active"))
        ]
        restore_blob = previous_versions[0] if previous_versions else None

        with transaction.atomic():
            pdf_repository.mark_blob_superseded_for_retention(
                blob_id=active_blob_id,
                section_code=current.get("catalog_section_code"),
                is_class_tracked=bool(current.get("catalog_is_class_tracked")),
                retain_all_versions=bool(current.get("catalog_retain_all_versions")),
            )
            removed_blob = pdf_repository.get_blob(active_blob_id) or active_blob

            if restore_blob:
                restored_blob = pdf_repository.reactivate_blob(str(restore_blob.get("blob_id")))
                update_values = {
                    "pdfAttachmentId": str(restored_blob.get("blob_id")),
                    "pdfMissing": False,
                    "certificateNumber": None,
                    "issuingAuthority": _default_issuing_authority(current),
                    "placeOfIssue": None,
                    "issueDate": None,
                    "expiryDate": None,
                    **_auto_accepted_tracked_item_values(_blob_ocr_payload(restored_blob or restore_blob)),
                }
                restored_blob_response = serialize_pdf_blob(restored_blob)
            else:
                update_values = {
                    "pdfAttachmentId": None,
                    "pdfMissing": True,
            ...
            serialized_before = serialize_tracked_item(before) if before else None
            serialized_after = serialize_tracked_item(after)
            serialized_removed_blob = serialize_pdf_blob(removed_blob)
            record_audit_event(
                actor=request.user,
                action="remove_pdf",
                entity_type="pdf_blob",
                entity_id=active_blob_id,
                vessel_id=vessel_id,
                before=serialize_pdf_blob(active_blob),
                after=serialized_removed_blob,
                reason=reason,
                metadata={
                    "source": "api.certs.tracked_items.remove_pdf",
                    "tracked_item_id": str(tracked_item_id),
                    "restored_pdf_blob_id": restored_blob_response.get("id") if restored_blob_response else None,
                },
            )
            record_audit_event(
                actor=request.user,
                ...
                after=serialized_after,
                reason=reason,
                metadata={"source": "api.certs.tracked_items.remove_pdf"},
            )
            record_cert_change_log(
                tracked_item_id=serialized_after["id"],
                before=before,
                ...
                version_after=int(after.get("version") or 1),
                actor=request.user,
                source_ref="api.certs.tracked_items.remove_pdf",
            )

        return Response(
            {
                "trackedItem": serialized_after,
                "removedPdfBlob": serialized_removed_blob,
                "restoredPdfBlob": restored_blob_response,
            }
        )
...
def _blob_ocr_payload(blob: dict) -> dict:
    payload = blob.get("ocr_payload_json")
...
def _default_issuing_authority(item: dict) -> str:
    value = str(item.get("catalog_issuing_authority_type") or item.get("issuing_authority") or "company").strip()
    return value.title() if value else "Company"
