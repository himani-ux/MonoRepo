...
class PdfBlobRepository:
    ...
    def get_blob_for_tracked_item_sha(self, *, tracked_item_id: str, content_sha256: str) -> dict[str, Any] | None:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                {limit_clause}
                """,
                [tracked_item_id, content_sha256],
            )
            return _fetch_one(cursor)
    ...
    def reactivate_blob(self, blob_id: str) -> dict[str, Any]:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                UPDATE dbo.vims_certs_pdf_blob
