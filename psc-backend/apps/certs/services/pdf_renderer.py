...
class ReportLabPdfRenderer:
    ...
    def _render_register(
        ...
        *,
        ...
        watermark_text = _watermark_text(payload, include_recipient=False)
        story.append(Spacer(1, 3 * mm))

        if not rows:
            story.append(Paragraph("No certificates match this print selection.", styles["CertBody"]))
        elif manifest_only:
        ...
        if watermark_text and not manifest_only:
            on_page = _bottom_right_watermark(watermark_text)
...
def _build_print_table(rows: list[dict], styles: dict[str, ParagraphStyle]) -> Table:
    header = ["Section", "Sub No.", "Certificate / Survey", "Cert No.", "Issued By", "Issue", "Expiry", "Last Done", "Next Due", "Status"]
    data = [header]
    ...
    table = Table(data, repeatRows=1, colWidths=[25 * mm, 12 * mm, 56 * mm, 27 * mm, 27 * mm, 22 * mm, 22 * mm, 22 * mm, 22 * mm, 28 * mm])
...
def _vessel_header(rows: list[dict], payload: dict) -> str:
    if not rows:
        return "No vessel details available."
    ...
    return f"Vessels: {len(vessel_names)}"


def _watermark_text(payload: dict, *, include_recipient: bool = True) -> str:
    ...
    if watermark == "AUDIT_COPY":
        return f"AUDIT COPY - {recipient}" if include_recipient and recipient else "AUDIT COPY"
    if watermark == "MASTER_COPY":
        return f"MASTER COPY - {recipient}" if include_recipient and recipient else "MASTER COPY"
    if watermark == "DRAFT":
...
def _bottom_right_watermark(text: str):
    def draw(canvas, document) -> None:
        ...
        canvas.drawRightString(width - 10 * mm, 5 * mm, text)
        canvas.restoreState()
