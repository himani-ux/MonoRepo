...
**Incident simplification validation update (2026-07-13; PDF wording current as of CR-082 on 2026-07-13):** For incident records, current validation follows the simplified flow in `safety_ssot/VIMS-SAFETY-MODULE-SSOT.md` section 3.0. Binding rules are: narrative minimum still applies; occurred/reported timestamps cannot be in the future or reversed; vessel scope is enforced; the Safety Incidents register filters by Vessel, `risk_band`, and Status, where the Vessel dropdown is populated from active `VesselData` rows within the user's scope and selected ships are sent as `vessel_id`; Phase 1 Vessel and Vessel code are system-derived identity fields and are disabled after autofill/save; reporter identity is required; risk band is required; investigation depth is derived from risk band; office communication is required as yes/no using "Was office informed?", and mode is required only when yes using "How was office informed?"; the current mode dropdown offers On call and On email only; Report time and Shore Assistance Required are visible together on one Phase 1 row; latitude and longitude are visible together on their own Phase 1 row and stored on the incident record; Shore Assistance Required, Location of Vessel, Location of Vessel detail, Location on Board, Risk Assessment carried out, Toolbox Meeting carried out, Permit Issue, Type of Activity, Departure Date, and Vessel Condition are nullable visible Phase 1 incident-reporting context fields, with the four operational fields placed above Departure Date and Vessel Condition. Location of Vessel is limited to At Sea (Open sea condition), At Sea (Coastal passage), In Port, and At Anchorage; detail is retained only for In Port / At Anchorage. Incident Type Other requires typed details when Other is selected. Last Port remains legacy storage only and is not shown, sent by the current frontend, or printed in the current Incident PDF; legacy injury `what_happened_narrative` remains storage/API compatibility only and is not shown as a current injury Investigation - Narrative field or printed in the current Incident PDF; Weather ice-condition fields remain legacy storage only and are not shown or sent by the current Weather Condition UI; up to three loss types are allowed including `Other`; `Other` requires text. Corrective Action and Preventive Action are separate visible screens and validate user-visible Description plus optional Due Date; they do not collect or require status/open-close, owner/checker, risk-reduction, theme, effort, or remaining-risk fields. Corrective and Preventive screens allow multiple visible action rows when a description is complete. Office Review send-back validates only the rework comment and permission in the current UI, is not blocked solely by internal `current_phase` being below 7, sends the fixed action-rework target, and displays the latest typed comment as a red Rework summary while the incident remains sent back. Ship-side and office-side users can mark Rework Done, which transitions backend phase `6 -> 7` and state `SENT_BACK -> UNDER_REVIEW`. Ship-side Office Review shows a pending message when no office comment exists. Office Review Accept / Close closes the incident for PIC or DPA. Loss Evaluation save is available to authorized ship-side and office-side users with incident form access and vessel scope without waiting for Office Review approval; the current UI requires the user to select Incident Report or Injury Report first and persists that selection as nullable `report_type`; Loss Evaluation does not own closure and has no Close Incident controls. Incident PDF preview/download and MSC-MEPC.3/Circ.4 export are not blocked solely by pending Phase 7 acceptance; record-type and regulatory applicability checks still apply. Current Incident PDF output places Corrective Actions and Preventive Actions before Evidence (Documents), starts Loss Evaluation on a new page when selected, labels onboard location as Location on Board, prints **Describe What happened?** after **Reporter Details** and before **Injury Details** when the title is **Injury Report**, prints the same narrative below intake/detail sections immediately before **Root Cause Analysis** for standard **Incident Report** output, prints Location of Vessel once with any In Port / At Anchorage detail appended in the same value, does not print a separate Specific vessel location row, does not print legacy Last port, separates Corrective Actions from Preventive Actions, renders each action as a full-width bordered row/box without a left-side `Description` label column, prints the description first, prints any saved linked due date second as `Due Date: YYYY-MM-DD`, groups Immediate Cause and Root Cause once each with cause factor on the left and cause/reason on the right, prints Witness Statements as separate blocks with clickable downloadable attachment links, without repeating a Witness name row, and without printing the old free-text `What the witness said` value, prints Office comments/ lesson learnt as one undivided full-width comment box from the stored office comment text with typed line breaks and repeated spaces preserved and no artificial chunk rows, and prints only Reporter plus PIC / DPA office signature rows. The former Lessons Learned screen is removed from current navigation and its old route redirects to Office Review. The former Phase 1 first-check gate is removed from the current UI/API/PDF flow under `D-MAINT-CR018`. User-entered IMO classifier, resource handoff, link-to-existing-finding, and mandatory bias-filter gates are superseded for the simplified V1 UI.
...
| V-INC-005A | User views or edits Phase 1 after vessel identity is auto-filled or saved | UI/server | Vessel and Vessel code are disabled in the UI; backend still validates `vessel_id` against vessel scope and ignores display-only code edits on update | D-MAINT-CR054 |
...
| V-INC-018 | Phase 1 injury estimated-cost fields are present in legacy data | none for current UI; frontend strips from Phase 1 submit payload | Phase 1 does not show estimated-cost controls; enter current cost details in Phase 7 Loss Evaluation. | D-MAINT-CR055 |
...
|--------|--------------------------------|--------------------|
| Corrective Action | Description is required before save. Due Date is optional. | Status/open-close, owner/checker, and completion/verification fields are not user-facing for incident corrective action capture. |
| Preventive Action | Description is required before save. Due Date is optional. | Status/open-close, risk reduction, Remaining risk, risk-confirmation checkbox, theme, effort, and "Prevent It Happening Again" wording are not user-facing for incident preventive action capture. |
...
Editing a saved Corrective Action or Preventive Action card reuses the same visible-field validation and PATCHes the selected existing recommendation/action row. Saving without using Edit creates another row, and multiple active rows per incident/tier are allowed.
...
| Rule ID | Trigger condition | Enforcement point | Error message | Decision ref |
|---------|-------------------|-------------------|---------------|--------------|
| V-INC-060 | Corrective or Preventive Action missing Description | client + server | "Enter a description before saving." | D-MAINT-CR065 |
| V-INC-061 | Closure attempt on RED/YELLOW without ALARP attestation flag `true` on every legacy System-Action | legacy compatibility only | Current incident Office Review does not block description-only preventive rows on ALARP attestation fields. | D-GAP-R02; D-MAINT-CR065 |
| V-INC-062 | Preventive Action saved without theme, effort, residual-risk wording, risk reduction, or due date | none | These fields are not current user-facing fields for incident preventive action capture. | D-GAP-R02; D-MAINT-CR049; D-MAINT-CR065 |
...
| Visible Phase 3 → 4 | Corrective Action saved with Description | D-MAINT-CR038, D-MAINT-CR065 |
| Visible Phase 4 → 6 | Preventive Action saved with Description; frontend continues directly to Office Review | D-MAINT-CR038, D-MAINT-CR042, D-MAINT-CR043, D-MAINT-CR049, D-MAINT-CR052, D-MAINT-CR065 |
...
| Visible Phase 6 → CLOSED | Office Review accepted/closed by PIC or DPA for any risk band; optional Office Comments/lesson learnt saved without a word limit; ship-side pending message displays when no comment exists; PDF download is not blocked solely by pending Phase 7 acceptance, is visible to ship-side and office-side users, and exposes only the optional Print Loss Evaluation checkbox because the normal report sections are compulsory. Send for rework uses a comment plus the fixed action-rework target, does not require internal `current_phase = 7` before recording rework, and the latest comment is visible as a red Rework summary while the incident remains sent back. Ship-side and office-side users can mark Rework Done, moving backend phase `6 -> 7` and state `SENT_BACK -> UNDER_REVIEW`. | D-PDF-01, D-MAINT-CR042, D-MAINT-CR043, D-MAINT-CR044, D-MAINT-CR049, D-MAINT-CR050, D-MAINT-CR056, D-MAINT-CR058, D-MAINT-CR064, D-MAINT-CR074, D-MAINT-CR075, D-MAINT-CR079 |
| Visible Phase 6 Fleet Alert | Fleet Alert opens a vessel-selection popup populated from active `VesselData` ships; at least one ship is selected before Confirm; Select all vessels selects every listed vessel; frontend advances the legacy backend phase to Office Review before loading recipient ships when needed; backend accepts Office Review and later final phases for this notification action while blocking early investigation phases; in-app and email dispatch is scoped only to selected ships; incident fleet-alert email CCs `HSSEQ@kaizenship.net`, attaches the Incident PDF, and uses a short prevention-focused body | D-MAINT-CR051, D-MAINT-CR061, D-MAINT-CR062, D-MAINT-CR064, D-MAINT-CR086 |
| Visible Phase 7 save | Authorized ship-side and office-side users with incident form access and vessel scope can save the Loss Evaluation row in `vims_safety_incident_loss_evaluation` after choosing Incident Report or Injury Report; this is additional data entry and not a close gate | D-MAINT-CR052, D-MAINT-CR053, D-MAINT-CR056 |
...
| V-INC-091 | User attempts to close an incident from Phase 7 Loss Evaluation compatibility endpoint | server | "Incident close is handled in Phase 6 Office Review." | D-MAINT-CR056 |
| V-INC-092 | User attempts to find Phase 7 closing note / Close Incident controls in the current UI | UI | no closing controls are rendered on Loss Evaluation | D-MAINT-CR056 |
...
| V-INC-095 | Incident Fleet Alert submitted with no selected ship, an unknown ship, or an inactive/deleted `VesselData` row | UI/server | Confirm disabled until at least one ship is selected; server returns "Select at least one ship." or field-level selected-ship error | D-MAINT-CR051, D-MAINT-CR062 |
...
| V-INC-097 | Name of master or Name of Chief Engineer is blank when Phase 7 Loss Evaluation loads | server payload | blank fields default from current active onboard `Crew_Onboarding_History` + `HRM501` + `master_applied_rank` records for the incident vessel; nonblank saved Loss Evaluation names are preserved | D-MAINT-CR063 |
