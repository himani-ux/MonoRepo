...
> **Stage 3A transactional UUID reference readiness (2026-05-21):** Safety transactional tables still retain integer `id` as the actual primary key and UUID `public_id` as the transitional public identifier. Child/reference rows now carry UUID shadow reference columns beside their legacy integer references, for example `incident_uuid`, `meeting_uuid`, `inspection_uuid`, `finding_uuid`, `recommendation_uuid`, `source_uuid`, and `parent_uuid`. These columns are backfilled from parent `public_id` values where the legacy parent row exists and are kept nullable so historical audit rows with missing/deleted parents do not block runtime. Final promotion to UUID `id`, preservation of old integer values as `legacy_int_id`, and removal of `public_id` remain a later stage only after all references are UUID-clean.
