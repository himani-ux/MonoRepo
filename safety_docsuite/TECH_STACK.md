...
This document is the **version-lock specification** for the VIMS Safety Module — the migration of the KSM safety management system (Incident Reporting, Near Miss Reporting, Safety Committee Meeting, Safety Officer Inspection) from the legacy eMarineSoft platform onto the VIMS monorepo at `apps/safety/` (Django backend) and `src/routes/safety/` + `src/components/safety/` (React frontend), against the shared database `ksm_marine_live`.
...
- All versions are pinned with `==` (Python) or exact strings (package.json) — no `^`, `~`, `>=`, or `*`.
...
> **Rationale:** see `VIMS-Reporting-Module/TECH_STACK.md` §1–§7 for full rationale on each choice. Frontend parity with the Inspection module is a platform rule (LESSONS `VIMS-Reporting-Module/CLAUDE.md` §Forbidden Actions → Database / Auth / Frontend).
...
Permission model: permissions live in the shared `msc_profiles` table as `SAF_F_*` (forms — Incident / Near Miss / SCM / SOI) and `SAF_P_*` (processes — Create / Submit / Send-back / Approve / Close). No new permission table. Vessel scoping reuses `master_RoleByVessel` (office) + `Crew_Onboarding_History` (ship).
...
Database: **`ksm_marine_live`** (shared instance, SQL Server). Safety uses the same connection as Reporting / Inspection — the DB router registers no new DB alias.

### 1.4 File Generation / Parsing

| Package | Version | Safety use |
|---------|---------|-----------|
| reportlab | 4.2.0 | Primary PDF renderer — see §2.1 |
...
| requests | 2.32.3 | Slack webhook dispatch (per D-GAP-F2 — best-effort) |
...
| react-router-dom | 6.24.0 | Client routing — mounts `/safety/incidents`, `/safety/near-miss`, `/safety/scm`, `/safety/soi` |
...
| Package | Version | Safety use |
|---------|---------|-----------|
| @tanstack/react-query | 5.51.1 | Server state — all `/api/safety/*` fetches |
| zustand | 4.5.4 | Client state — incident draft, SOI selected areas, Ad-Hoc SCM trigger state |
...
| zod | 3.23.8 | Schema validation — `src/schemas/safety/*` per form, `schema_version` column on `vims_safety_incident` |
...
| idb | 8.0.0 | IndexedDB wrapper — 30-second form auto-save (D-GAP-F1) for Incident, Near Miss, SCM agenda, SOI finding-registration |

**Safety offline posture:** auto-save every 30 seconds to IndexedDB (D-GAP-F1). On reconnect, form resumes from last saved state. This is a **draft-only** guarantee — final submission requires connectivity. **Paper-first SOI (D-GAP-E4) does NOT use background sync**; see §6.

### 1.17 Mobile (Flutter) — not used in V1
...
### 2.1 PDF Renderer — 10-Section Template (D-PDF-01, D-PDF-02, D-PDF-03a, D-PDF-03b)

| Package | Version | Purpose | Install Command |
|---------|---------|---------|-----------------|
| reportlab | 4.2.0 | Primary PDF engine (inherited) — renders 10-section incident report template, near-miss 1–2 page template, SCM 10-section legacy `vw_GetSCM_Master` layout, SOI summary record | `pip install reportlab==4.2.0` |
...
**Decision:** ReportLab is chosen (not WeasyPrint — banned at platform level per `VIMS-Reporting-Module/TECH_STACK.md` §13). PyPDF2 is the companion manipulation layer — both already platform-installed; no new dependency added for Safety.
...
**D-PDF-03a near-miss template:** distinct lighter 1–2 page layout (what-happened + suggestion + immediate action). No investigation / cause-tree content.
...
### 2.2 Barcode / QR Library — SOI Unique Checklist ID
...
| Package | Version | Purpose | Install Command |
|---------|---------|---------|-----------------|
| qrcode | 7.4.2 | QR code generation for SOI PDF/Excel checklist unique ID (links printed paper → digital record per D-GAP-E4) | `pip install qrcode==7.4.2` |
...
> **Gap:** D-SOI-10 (revised) + D-GAP-E1/E3/E4 mandate a unique checklist ID that survives paper→digital linkage, but the visual encoding is explicitly deferred to build-time (Round 20).
...
### 2.3 Full-Text Search (FTS) Engine — BLOCKED
...
|---------|--------|-------------|
| No ETL from Reporting → Safety | Same DB, same instance (D-GAP-I2) | Safety service layer issues JOIN queries against `vims_noon_report` / `vims_voyage` directly |
...
- No Kafka, RabbitMQ, or message broker added for Safety. Celery + Redis (§1.5) handles async work internally.
...
Inherited verbatim from the VIMS platform / Reporting Module TECH_STACK.md §7 — Safety does **not** provision its own infrastructure.

### 4.1 Database Host

| Component | Value |
|-----------|-------|
| Engine | SQL Server — same instance as Inspection + Reporting |
...
|-----------|-------|--------|
| Cloud provider | Azure | Platform (Reporting Module §7.1) |
...
| Near-miss attachments | `/var/www/ksm_uploads/safety/{vessel_id}/near-miss/{nm_id}/` | — |
...
VITE_SAFETY_API_BASE_URL=http://localhost:8000/api/safety

# Safety Module feature flags
VITE_SAFETY_FTS_ENABLED=false   # Locked off until Phase 8 FTS resolution (BLOCKED §2.3)
VITE_SAFETY_QR_FORMAT=qr        # qr | code128 — default QR, resolves BLOCKED §2.2
```
...
## 5. Monitoring / Backup / Performance — Platform Inherited
...
3-year hard-delete on incident + near-miss + SCM + SOI records. Celery Beat schedules the retention job (`django-celery-beat` 2.6.0). No legal-hold feature — DPA is responsible for out-of-band export before the 3-year mark when a case is open.

### 5.4 Performance Posture (D-GAP-H1)

No formal concurrent-user load target at module level. Inherits VIMS platform baseline. No module-level decision between pessimistic single-editor lock vs optimistic-locking — Safety follows whatever the platform provides (currently optimistic-locking via `updated_date` + `schema_version` columns on `vims_safety_*` tables).
...
|---------|-----------------|
| Chrome | 90+ |
| Safari | 14+ (iOS vessel tablets — mobile-first mandate) |
| Edge | 90+ |
| Firefox | 90+ |
...
|--------------------------|--------|
| Scan upload of filled checklist | D-GAP-E4 — paper filed in ship SMS filing system; digital record linked only via unique checklist ID |
| Upload column on `vims_safety_soi_inspection` for paper scan | D-GAP-E4 — no scan endpoint, no upload column |
| Background sync of filled checklist answers | D-GAP-E4 — Yes/No/NA item responses live on paper only; not in the DB |
| Service-worker-queued checklist response submission | D-GAP-E4 — no such endpoint exists |
...
- HIGH-severity finding photos (D-GAP-M24 mandates ≥1 photo)
- Inspection event metadata (event ID, vessel, date range, areas selected, trainees if any — up to 3)
- Safety Officer + Assistant digital signatures at save; Master counter-signs at approval (D-GAP-M15)

These use IndexedDB 30-second auto-save (D-GAP-F1) + `workbox-background-sync` 7.1.0 for submission once reconnected — **findings only**, never the paper checklist itself.
...
|-------------|-----------|---------------|------|--------------------|-------|
| **Slack Web API** | Safety → Slack (outbound only) | `https://slack.com/api/chat.postMessage` (HTTP v1, no dated version) | Bot token (OAuth 2.0), env var `SLACK_BOT_TOKEN` | D-GAP-F2 | Best-effort; in-app notification is authoritative. Rate limit 1 msg/sec per channel |
| **`master_notification` (platform queue)** | Safety → Platform (outbound) | Internal VIMS (DB-row write) | Django ORM writes | `<vims_integration>` | Safety writes notification rows; platform notifier consumes |
| **Reporting Module — MSC-MEPC.3 position** | Safety ← Reporting (live SQL join) | Internal (live join on `vims_noon_report`) | Same-DB (SimpleJWT for user context) | D-GAP-M09 | ±12 hours tolerance from incident timestamp; user may edit auto-fill |
| **Reporting Module — Daily Report missing** | Safety ← Reporting (live SQL join) | Internal | Same-DB | D-GAP-M10 | Manual lat/long accepted; record flagged `awaiting_daily_report_match`; never blocks submit |
| **WRH Module — SCM attendance** | Safety ← WRH (live SQL join) | Internal (live join on `wrh_attendance`) | Same-DB | D-GAP-M11 | Missing data warns, does not block. Row flagged "WRH data unavailable" |
| **WRH Module — timezone config** | Safety ← WRH (live SQL join) | Internal (live join on `wrh_ship_time_config`) | Same-DB | D-GAP-M26 | UTC storage + vessel local time resolution per Reporting pattern |
| **CMS — SOI assistant lookup** | Safety ← CMS (live SQL join) | Internal (live join on `Crew_Onboarding_History`, `HRM501`) | Same-DB | D-GAP-I2 | No staleness; same DB |
| **Purchase Module — CA hard FK** | Safety ↔ Purchase (live + FK) | Internal (`purchase_req_id` FK) | Same-DB | D-GAP-M12 | Referential integrity; requisition cannot be archived while linked to open CA |
| **PMS Module** | **DECOUPLED — no integration** | N/A | N/A | D-GAP-I1 | No FK, no live join, no API. Manual cross-reference only |
| **VIMS Circular Module** | Safety → Circular (live SQL write) | Internal (write to VIMS Circular table) | Same-DB | §1, Lessons Learned | Fleet circular draft auto-linked post-incident closure |
| **IMO flag-state notification** | Out-of-band (no system integration) | N/A — DPA handles manually | N/A | D-GAP-G1 | MSC-MEPC.3/Circ.4 PDF auto-export (D-DNV-12) supports DPA; no deadline tracking in V1 |
| **Class society notification** | Out-of-band | N/A | N/A | D-GAP-M13 | DPA handles manually, same pattern as G1 |
| **Open-Meteo Marine API** | Not consumed by Safety | N/A | N/A | — | Reporting-only integration |
| **UN/LOCODE Port Database** | Safety ← `master_port` (shared) | Internal (SELECT from `master_port`) | Same-DB | Platform | Safety reuses for SCM location, SOI port field, incident location |
| **Firebase FCM / APNs** | Not consumed by Safety V1 | N/A | N/A | — | Safety V1 does not push to mobile; `master_notification` is sole channel |
...
| Slack API | Existing KSM workspace, bot token | $0 | Shared existing workspace | `VIMS-Reporting-Module/TECH_STACK.md` §6.2 |
| Firebase Cloud Messaging | Not used by Safety V1 | $0 | — | D-GAP-F2 (in-app notification authoritative) |
| Open-Meteo Marine API | Not used by Safety | $0 | — | Reporting-only |
...
| FTS engine (when Phase 8 resolves BLOCKED §2.3) | Elasticsearch cluster if chosen; $0 if native SQL Server FTS | TBD at resolution — deliberately unresolved per Round 20 | — | BLOCKED §2.3 |
...
| FTS engine selection (Phase 8) | Platform + Safety Tech Lead | Resolves BLOCKED §2.3; triggers IMPLEMENTATION_PLAN Phase 8 step |
| SOI unique-ID format selection | Product + Design | Resolves BLOCKED §2.2; template-only change |

### 9.2 Forbidden Packages (Safety-Specific Additions to Platform List)

In addition to the forbidden list in `VIMS-Reporting-Module/TECH_STACK.md` §13, Safety specifically forbids:

| Package | Reason | Use Instead |
|---------|--------|-------------|
| Any hash-chain / blockchain library | D-GAP-D2 — no crypto in V1 | `vims_safety_field_history` audit table |
| Any PKI / X.509 / UETA library | D-GAP-D1 — hybrid digital + wet-signed-scan only | Typed-name + timestamp + device fingerprint |
| `pytesseract` / OCR libs | D-GAP-E4 — no scan upload, no OCR needed | Paper stays on paper |
| `elasticsearch-py` / `opensearch-py` (pre-resolution) | BLOCKED §2.3 — FTS engine not yet chosen | LIKE-based narrow search as V1 fallback |
...
All platform packages (§1) are installed via `VIMS-Reporting-Module/TECH_STACK.md` §9.2 `requirements.txt`. Safety adds these two lines:
...
**None.** All frontend packages (§1.7–§1.16) inherit verbatim from Reporting. Safety's `src/components/safety/` and `src/routes/safety/` consume the same shadcn/ui + Radix + react-hook-form + Zod stack.
...
Safety's compatibility envelope is **identical** to Reporting's plus the two Safety-specific Python lines in §10.1. Any divergence is a violation.
...
|-----------|--------|----------|
| Every dependency has exact semver (no "latest", no `^`, no `~`) | PASS | §1 all tables use `X.Y.Z` form; §10.1 uses `==` |
| Hosting + region specified | PASS | §4.1–§4.2: SQL Server, `southindia` primary, `centralindia` standby |
| Every integration documented with API version | PASS | §7 Integration Matrix: Slack HTTP v1, internal same-DB joins, UN/LOCODE bulk, Open-Meteo explicitly not-consumed |
| Cost estimate per service | PASS | §8 all 12 service lines; total ~$5/month incremental |
| No "TBD" / "recommended" | PASS | FTS cost line marks "TBD at resolution — deliberately unresolved per Round 20" which is the BLOCKED stub (not a gap) |
| FTS engine flagged as Round 20 build-time deferral (BLOCKED stub) | PASS | §2.3 BLOCKED stub |
| No crypto libs | PASS | §2.4 explicit exclusion table |
| PMS decoupled (D-GAP-I1) | PASS | §3.1, §7 Integration Matrix row |
| Paper-first SOI no upload / sync language | PASS | §6.2 explicit NOT-supported list |
...
| Django `apps.safety` / `/api/safety/` / `ksm_marine_live` referenced | PASS | §1.1 `apps.safety`; §1.2 `/api/safety/`; §4.1 `ksm_marine_live` |

### 12.1 BLOCKED Stubs Summary

| # | Section | Label | Resolution owner | Required by |
|---|---------|-------|------------------|-------------|
| 1 | §2.2 | SOI unique-ID flag format | Product + Design | IMPLEMENTATION_PLAN Phase 4 |
| 2 | §2.3 | FTS engine selection | Platform lead | IMPLEMENTATION_PLAN Phase 7 |
...
| Document | Reference |
|----------|-----------|
| `VIMS-Reporting-Module/TECH_STACK.md` | Platform tech stack — inherited verbatim for all non-Safety-specific lines |
| `VIMS-SAFETY-MODULE-SSOT.md` | 159 locked decisions (D-*, D-GAP-*) — authority on requirements |
| `VIMS-Safety-Module/PRD.md` | Features (FEAT-SAF-*) — maps to this TECH_STACK via library choices |
| `VIMS-Safety-Module/BACKEND_STRUCTURE.md` | Schema + APIs; contains the 12-row build-time deferrals table referenced by §2.2 and §2.3 |
...
| `VIMS-Safety-Module/VALIDATION_RULES.md` | Input + compliance rules; uses Zod 3.23.8 (§1.13) |
| `VIMS-Safety-Module/IMPLEMENTATION_PLAN.md` | Master blueprint; Phase 8 resolves BLOCKED §2.2 and §2.3 |
| `VIMS-Reporting-Module/CLAUDE.md` | Platform forbidden-actions list; Safety inherits |
| KSM SSQE Manual Rev 01 Feb 2026 §9, §11 | Regulatory authority for SCM (§9) and Incident (§11) |
| ISM Code 2010 amendments §10 | Audit trail non-repudiation satisfied via `vims_safety_field_history` per D-GAP-D2 |
