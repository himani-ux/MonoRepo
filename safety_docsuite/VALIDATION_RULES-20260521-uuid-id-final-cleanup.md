...
**Stage 3A UUID reference validation note:** UUID shadow reference columns on Safety-owned transactional child rows are migration-readiness fields, not new user inputs. Validators must continue to apply the existing legacy integer/public UUID route resolution and business rules. When a child row can resolve its legacy parent, its UUID shadow reference should match the parent `public_id`; unresolved historical/polymorphic rows may keep a null UUID shadow reference and must not create a new workflow blocker.
