...
**Stage 3A UUID reference readiness:** Transactional route behavior is unchanged. Backend child/reference rows now maintain UUID shadow references beside legacy integer references so future UUID `id` promotion can happen without changing user journeys. Screens must continue to use existing route identifiers and public UUIDs where already available; no workflow, lifecycle, validation, permission, PDF, dashboard, export, or UI action changes are introduced by Stage 3A.
