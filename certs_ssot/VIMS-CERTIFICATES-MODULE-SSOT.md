...
| D-CERT-206 | Supersedes D-CERT-066 only where BV report parsing or docs treated BV Class Memoranda or Statutory Memoranda as Conditions of Class. BV `conditions_of_class[]` must be populated only from the actual BV Conditions of Class section; BV memoranda are not COC and must not create Conditions of class review flags. | LOCKED |
| D-CERT-207 | Supersedes D-CERT-066 and D-CERT-206 for all supported class societies where non-COC sections were treated as Conditions of Class. NK, KR, and BV `conditions_of_class[]` must be populated only from exact Condition of Class / Conditions of Class source sections; KR Actionable Note, KR Statutory Condition, NK Condition of Installation, NK Condition of Statutory Survey, BV Class Memoranda, and BV Statutory Memoranda must not create Conditions of class review flags. | LOCKED |
...
| D-CERT-208 | **Normal Print Vessel Status UI simplified.** Supersedes D-CERT-138, D-CERT-140, and D-CERT-149 only for the normal `/certs/print` user-facing form. The screen label is "Print Vessel Status"; it uses the current vessel context and one single-select "Certificate List" dropdown. Option "Sections" prints the vessel status by sections; selecting one certificate prints that certificate. Normal UI does not show Buckets/Add Vessel filters, Scope, Status, Sections, Watermark, or Recipient fields. Backend/API artifact fields, Print History, Share Bundle, auditor export, and email delivery support remain compatible. | LOCKED |
| D-CERT-209 | **Normal print label = "Print certs status".** Supersedes D-CERT-208 only for the visible screen/action label. The normal `/certs/print` workflow remains current-vessel scoped with one single-select "Certificate List" dropdown and the same removed controls from D-CERT-208. | LOCKED |
| D-CERT-210 | **Normal print/share selection = certificate sections, not individual certificates.** Supersedes D-CERT-208/D-CERT-209 only for selection contents. `Print certs status` shows a single "Certificate List" dropdown containing `All sections` plus vessel certificate sections; choosing one section prints that section. `Share Bundle` uses a section multi-select with Select all / Clear all and bundles all certificates in selected sections. Backend-compatible `customCertIds` remains accepted for integrations/history, but the normal UI does not show individual certificate choices. | LOCKED |
| D-CERT-211 | **Print certs status section dropdown label = "Certificate sections".** Supersedes D-CERT-210 only for the visible dropdown label. The selection behavior remains `All sections` plus vessel certificate sections. | LOCKED |
| D-CERT-212 | **Normal visible Excel omits internal print metadata.** Supersedes D-CERT-128, D-CERT-141, D-CERT-147, and D-CERT-202 only where they could imply that `print_id`, print scope, or system-state hash must be printed inside the generated Excel workbook. The Excel workbook remains data-only for analysis and keeps certificate rows, while `print_id`, scope, system-state hash, timestamp, user, blob links, filenames, Print History, and audit records remain stored for traceability. | LOCKED |
